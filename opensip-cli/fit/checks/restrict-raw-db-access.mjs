/**
 * @fileoverview restrict-raw-db-access — confine the raw Drizzle handle
 *               (`DataStore.db`) to the persistence ownership boundary.
 *               Project-local SELF-check.
 *
 * Relocated out of `@opensip-cli/checks-*` (placement sweep) because it encodes
 * opensip-cli local facts: it cites ADR-0009 + the `tables-only-in-persistence`
 * dependency-cruiser rule, hardcodes first-party persistence boundaries by path
 * (`packages/session-store/src/`, `packages/datastore/src/`), and is tightly
 * coupled to opensip-cli' own `DataStore.db` Drizzle seam. A consumer repo has
 * none of those facts, so the rule is opensip-internal, not universal. Inert for
 ...
 * does not trip unless it is used as a Drizzle query handle.
 *
...
function collectRawDbAliases(line) {
  const aliases = [];
...
function rawDbAliasQuery(alias) {
 ...
 */
export function analyzeRawDbAccess(content, filePath) {
