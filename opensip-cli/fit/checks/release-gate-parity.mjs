/**
 * @fileoverview release-gate-parity — the tag-driven release workflow's
 *               correctness gate must be AT LEAST AS STRICT as the PR gate.
 *               Project-local SELF-check.
 *
 * Relocated out of `@opensip-cli/checks-*` (placement sweep) because it encodes
 * opensip-cli local facts: it cites ADR-0017 (and ADR-0012), hardcodes the
 * first-party workflow path (`.github/workflows/release.yml`), and asserts the
 * presence/ordering of opensip-cli' OWN four PR-quality gate commands
 * (`pnpm lint`, `pnpm test:coverage`, `pnpm fit:ci`, `pnpm graph:ci`) relative
 * to this repo's `pnpm --filter "$pkg" pack` step. A consumer repo has none of
 * those facts, so the rule is opensip-internal, not universal. Inert for
 * adopters per `opensip-cli/fit/checks/README.md`.
 *
 * WHY (ADR-0017, "Release gate must be at least as strict as the PR gate"):
 ...
 * architecture gates in this set.
 */
 ...
 */
export function analyzeReleaseGateParity(content, filePath) {
