/**
 * @fileoverview live-runs-off-thread — keep live (TTY) runs executing OFF the
 *               main process (ADR-0028). Project-local SELF-check.
 *
 * Relocated out of `@opensip-cli/checks-*` because it encodes opensip-cli local
 * facts: it hardcodes opensip-cli' OWN filename conventions (`*-runner.tsx` for
 * a tool's live view, `*-worker.ts` for its headless engine entry), cites
 * ADR-0028, and keys on first-party seams — the off-thread selector
 * `runOffThreadOrInProcess` / the bare `createInProcessTransport` transport
 * (core) and the `persist*` session-write seam. A consumer that does not adopt
 * those conventions sees this check as a no-op. Inert for adopters per
 * `opensip-cli/fit/checks/README.md`.
 *
 * WHY: the original defect (the one ADR-0028 fixes) is a CPU-heavy tool
 * rendering its live view while the *same* main thread runs the engine: the
 * 80 ms clock + Ink reconciler starve under each synchronous chunk and the
 * spinner stutters. The fix routes every tool's live runner through the
 * off-thread selector `runOffThreadOrInProcess` (core), which forks the engine
 * to a headless worker subcommand and keeps the render process free. Two
 * regressions would silently undo it, and neither has an import edge
 * dependency-cruiser could catch:
 *
 ...
 */
export function analyzeLiveRunsOffThread(content, filePath) {
