/**
 * @fileoverview no-direct-stdout-in-tool-engine — tool engines must emit a
 *               SignalEnvelope, not write run output to stdout. Project-local
 *               SELF-check.
 *
 * Relocated out of `@opensip-cli/checks-*` (placement sweep) because it encodes
 * opensip-cli local facts: it hardcodes the first-party tool-engine path layout
 * (`packages/{fitness,graph,simulation}/engine/src/`), the ADR-0011 signal-output
 * currency, and opensip-cli's internal `ToolCliContext` seams (`cli.render` /
 * `cli.emitJson` / `cli.deliverSignals` / `cli.writeSarif`) plus the
 * `@fitness-ignore-file no-direct-stdout-in-tool-engine` directive. A consumer
 * repo has none of those packages or that output architecture, so the rule is
 * opensip-internal, not universal. Inert for adopters per
 * opensip-cli/fit/checks/README.md.
 *
 * WHY — ADR-0011 ("signal output currency"): a tool run emits a `SignalEnvelope`
 ...
 * are flagged.
 */
export function analyzeDirectStdout(content) {
  const violations = [];
