...
export const noLeakInternalImprovementPatterns = defineCheck({
  id: '610ee9f4-03c0-4d25-8d2d-4e98319d17f7',
    ...
    for (let i = 0; i < lines.length; i++) {
      if (badPatterns.some((re) => re.test(lines[i]))) {
        violations.push({
          line: i + 1,
          message:
            'Public docs page references an internal improvement process detail, local mechanism slug, or remediation artifact. These belong in local-only internal notes or are not user-facing. Move, remove, or add // public-docs-ok with justification if this is deliberate (e.g. high-level "the project uses local checks for self-improvement").',
          severity: 'warning',
        });
        break;
      }
