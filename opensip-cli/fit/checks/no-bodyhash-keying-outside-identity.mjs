/**
 * @fileoverview no-bodyhash-keying-outside-identity — the graph cross-shard
 *               MERGE layer may not key/stitch call edges by a bare
 *               `bodyHash`/`ownerHash`; it must route owner identity through
 *               the ONE shared module, `cli/orchestrate/edge-identity.ts`
 *               (ADR-0003). Project-local SELF-check.
 *
 * Lives here (not in the shipped `@opensip-cli/checks-*` packs) because it
 * hardcodes opensip-cli' OWN graph layout: the merge layer is named by
 * first-party path (`packages/graph/engine/src/cli/orchestrate/`), the one
 * allowed home is `edge-identity.ts`, and the compliant keying helpers
 * (`ownerEdgeKey` / `bucketEdgesByOwner` / `stitchEdgesByOwner`) are
 * first-party symbols. A consumer repo has none of those facts, so the rule is
...
function isMergeLayerFile(filePath) {
 ...
 */
export function analyzeNoBodyhashKeyingOutsideIdentity(content, filePath) {
 ...
 */
export async function analyzeAllNoBodyhashKeyingOutsideIdentity(files) {
  const violations = [];
