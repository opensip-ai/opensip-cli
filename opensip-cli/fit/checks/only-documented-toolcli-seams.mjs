/**
 * @fileoverview only-documented-toolcli-seams — host command handlers and tool
 *               engines must only use the documented seams on ToolCliContext
 *               (and its hostPlanes bag). Project-local SELF-check.
 *
 * Lives here (not in the shipped `@opensip-cli/checks-*` packs) because it
 * encodes opensip-cli' OWN architecture facts: it hardcodes first-party paths
 * (`packages/{fitness,graph,simulation}/engine/src/`, `packages/cli/src/commands/`,
 * the `bootstrap|cli-context|error-handler|report|index|welcome|ui` composition
 * root, the `session-store`/`datastore`/`/src/persistence/` boundaries), the
 * removed pre-scope handoff-bag symbols (`getCurrentRegistriesForScope`,
 * `setCliRuntimeContextForRun`, `getToolProvenanceForRun`, …,
 * `currentRuntimeContext`), and the `DataStoreFactory` construction shapes. It is
 * the static enforcement half of the "only documented seams" rule from the
 * host-planes-scope-seams-hygiene plan (Phases 3-4), citing ADR-0011, the
 * entered-scope invariant, and ADR-0036's host-owned baseline plane. A consumer
 * repo has none of those facts, so the rule is opensip-internal, not universal.
...
const matchesAny = (patterns, line) => patterns.some((p) => p.test(line));
...
export function analyzeOnlyDocumentedSeams(content, filePath) {
  ...
  const rules = PERSISTENCE_BOUNDARY.some((re) => re.test(filePath))
