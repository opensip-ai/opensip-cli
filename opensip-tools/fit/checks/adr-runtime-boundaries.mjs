...
function analyzeTreeSitterOwnership(content, filePath) {
  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (
      /\b(?:await\s+)?Parser\.init\s*\(|\bnew\s+Parser\s*\(|\bLanguage\.load\s*\(/.test(
        line,
      )
    ) {
      violations.push(
...
function analyzeHostOwnedVerdictRatchet(content, filePath) {
  ...
  if (
    !/^packages\/(?:contracts|fitness\/engine|graph\/engine|simulation\/engine)\/src\//.test(
      rel,
    )
  ) {
    return [];
...
function analyzeOneOutcomeShape(content, filePath) {
  ...
  if (
    !/^packages\/(?:cli|fitness\/engine|graph\/engine|simulation\/engine)\/src\//.test(
      rel,
    )
  ) {
    return [];
