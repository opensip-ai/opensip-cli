...
Tool-owned: [`packages/fitness/engine/src/tool.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/fitness/engine/src/tool.ts).
...
Tool-owned: [`packages/simulation/engine/src/tool.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/simulation/engine/src/tool.ts).
...
Tool-owned: [`packages/graph/engine/src/tool.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/graph/engine/src/tool.ts). The pipeline architecture and cache invalidation are documented in [`40-graph/01-stages-and-catalog.md`](/docs/opensip-cli/40-graph/01-stages-and-catalog/); perf-plan history is recoverable from `git -P log -- packages/graph`.
...
Tool-owned: [`packages/yagni/engine/src/tool.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/yagni/engine/src/tool.ts).
...
Tool-owned: [`packages/mcp/src/command.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/mcp/src/command.ts) (the bundled `@opensip-cli/mcp` tool, [ADR-0084](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/docs/decisions/ADR-0084-mcp-server-surface.md)).
...
Tool-owned (graph Tool). Read-only analysis of what changed and what depends on it — combining git change detection (or explicit files) with the persisted graph catalog. Rebuilds the catalog when missing ([ADR-0085](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/docs/decisions/ADR-0085-change-detection-substrate.md)).
...
CLI-owned: [`packages/cli/src/commands/init.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/cli/src/commands/init.ts).
...
CLI-owned: [`packages/cli/src/commands/host-subcommand-config.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/cli/src/commands/host-subcommand-config.ts). Operator workflows over the **project** `opensip-cli.config.yml` using the same composed schema the dispatcher validates at pre-action time. Distinct from `opensip configure`, which manages the user-global OpenSIP Cloud API key.
...
Both subcommands compose declarations from the live tool registry and admitted manifests — the same source as [`config-and-capabilities.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/cli/src/bootstrap/config-and-capabilities.ts). They do not open the SQLite datastore.

---

## `configure` — manage user-level settings

CLI-owned: [`packages/cli/src/commands/configure.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/cli/src/commands/configure.ts). Interactive — writes the OpenSIP Cloud API key to `~/.opensip-cli/config.yml` and verifies it best-effort against the cloud entitlement endpoint.
...
unfiltered envelope ([ADR-0085](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/docs/decisions/ADR-0085-change-detection-substrate.md)).
...
CLI-owned: [`packages/cli/src/commands/plugin.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/cli/src/commands/plugin.ts).
...
Adapters inherit the host-owned baseline ratchet ([ADR-0036](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/docs/decisions/ADR-0036-host-owned-baseline-ratchet-plane.md)) verbatim, the same as `fit`/`graph`:
...
The scanner's **raw native report** persists under `<project>/opensip-cli/.runtime/artifacts/<tool>/<runId>/` (host-owned, [ADR-0080](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/docs/decisions/ADR-0080-host-owned-artifact-write-seam.md)/[ADR-0091](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/docs/decisions/ADR-0091-external-scanner-finding-ingestion.md)): **`0600`** (owner-only), **gitignored**, and **never egressed** — only normalized, redacted `Signal`s leave the process. The host keeps the most-recent run-dirs per tool and prunes the rest after each write, governed by **`cli.artifacts.keep`** in `opensip-cli.config.yml` (default **10**; `0` disables pruning). Secret-scanner findings are **redacted** — only a short non-reversible preview reaches the signal, never the matched credential.
...
CLI-owned: [`packages/cli/src/commands/completion.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/cli/src/commands/completion.ts).
...
CLI-owned: [`packages/cli/src/commands/uninstall.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/cli/src/commands/uninstall.ts).
...
[`paths.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/core/src/lib/paths.ts),
[`logger.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/packages/core/src/lib/logger.ts)). Anything else in that
...
[ADR-0073](https://github.com/opensip-ai/opensip-cli/blob/v0.1.16/docs/decisions/ADR-0073-update-notification-policy.md)). The *fetch*
