...
The launcher's `decideReportOpen` ([`packages/cli/src/open-report.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/cli/src/open-report.ts)) returns `shouldOpen: true` only when **all** of these hold:
...
by [ADR-0155](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/docs/decisions/ADR-0155-canonical-audit-command.md); Run and
RunStep identity remains authoritative per
[ADR-0143](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/docs/decisions/ADR-0143-host-owned-run-step-ledger.md).
...
Source: [`packages/dashboard/src/client/overview.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/overview.ts).
...
Source: [`packages/dashboard/src/client/sessions.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/sessions.ts). Rendered inside each per-tool tab's Overview subtab; the tab switcher is in [`tool-tabs.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/tool-tabs.ts).
...
Source: [`packages/dashboard/src/client/checks.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/checks.ts).
...
Source: [`packages/dashboard/src/client/recipes.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/recipes.ts).
...
Source: [`packages/dashboard/src/code-paths.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/code-paths.ts) and the per-view browser modules under [`packages/dashboard/src/client/`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/) (`view-graph.ts`, `view-coupling.ts`, `view-distribution.ts`).

### Tool tabs

The report supports audit Run evidence plus fit, sim, graph, yagni, and installed Tool plugin runs. The top-of-page tab switcher filters the panels by view/tool. Change Impact joins a parent Run to its graph session; fit and sim use the shared Sessions/Catalog/Recipes shape, graph uses Code Graph with catalog exploration, and YAGNI uses Sessions/Detectors. Installed external scanner adapters fall into `External Tools`: their sessions render with the same Overview rows, run detail, verdicts, timing, findings, and tool badges as built-in tools, without requiring a custom dashboard module for each scanner. Source: [`tool-tabs.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/tool-tabs.ts) and [`tool-tabs-registrations.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/tool-tabs-registrations.ts).

---

## How it's generated

Static HTML. The generator ([`packages/dashboard/src/generator.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/generator.ts)) assembles:

1. The base HTML scaffold (head, body shell, the panel containers).
2. The CSS, inlined via `<style>` (from [`css.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/css.ts)).
...
[`packages/dashboard/src/generator.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/generator.ts)).
...
[`client/view-template.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/view-template.ts);
...
registration in [`code-paths.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/code-paths.ts).
...
[`tab-activators.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.5/packages/dashboard/src/client/tab-activators.ts) and
