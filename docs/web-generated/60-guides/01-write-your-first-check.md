...
By the end of this page you'll have: an installed CLI, an `opensip-cli.config.yml`, one custom check you wrote, a recipe that runs it, and a passing-or-failing exit code that CI can gate on. ~15 minutes if you're new to the tool; less if you've seen the [show-me page](/docs/opensip-cli/00-start/02-show-me-the-loops/).
...
If you wanted to *clean up* the violations first and gate on *new* ones only, this is where the baseline flow kicks in — see [wire into CI](/docs/opensip-cli/60-guides/03-wire-into-ci/) and [adopt in a monorepo](/docs/opensip-cli/60-guides/04-adopt-in-a-monorepo/).
...
The full GitHub Actions example is in [wire into CI](/docs/opensip-cli/60-guides/03-wire-into-ci/).

## Where to go next

| You want to … | Go to … |
|---|---|
| Ban a specific API in your codebase | [Ban an API pattern](/docs/opensip-cli/60-guides/02-ban-an-api-pattern/) |
| Add the GitHub Actions step | [Wire into CI](/docs/opensip-cli/60-guides/03-wire-into-ci/) |
| Graduate from `.mjs` files to a TypeScript workspace pack | [Adopt in a monorepo](/docs/opensip-cli/60-guides/04-adopt-in-a-monorepo/) |
| Reference the full check API | [Plugin authoring](/docs/opensip-cli/50-extend/01-plugin-authoring/) |
| See every built-in check | [Checks reference](/docs/opensip-cli/70-reference/05-checks-index/) |
