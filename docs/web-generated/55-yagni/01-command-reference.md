...
> **Scope (v0.1.12, ADR-0063):** yagni audits **config-surface reduction** (unused public config keys). It no longer performs duplicate-body detection — that re-implemented and diverged from graph's rule. **Duplicate / near-duplicate analysis now lives in [`opensip graph`](/docs/opensip-cli/40-graph/01-stages-and-catalog/)** (`duplicated-function-body`, `near-duplicate-function-body`). yagni owns no graph evidence; the `--graph` flag and `graphMode` config are deprecated and inert (removal in 0.1.13).
