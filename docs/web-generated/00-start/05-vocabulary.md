...
Checks are created with `defineCheck()` from [`packages/fitness/engine/src/framework/define-check.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/packages/fitness/engine/src/framework/define-check.ts), which returns a `Check` object ([`packages/fitness/engine/src/framework/check-types.ts:45`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/packages/fitness/engine/src/framework/check-types.ts)). A check's config carries an `id` (UUID, stable across renames), a `slug` (human-readable identifier), `tags`, a `description`, and one of three execution modes:
...
A **signal** is the canonical violation record. Every check produces `Signal[]`. Every renderer (table, JSON, SARIF, dashboard) consumes `Signal[]`. The shape lives in [`packages/core/src/types/signal.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/packages/core/src/types/signal.ts).
...
The kernel exports `createSignal(input)` from [`packages/core/src/types/signal.ts:53`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/packages/core/src/types/signal.ts) — that's how every check produces signals.

## Signalers config

The "signalers" name applies to a *configuration section*, not a rule primitive. The `signalers` directory ([`packages/fitness/engine/src/signalers/`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/packages/fitness/engine/src/signalers/)) owns the loader and Zod schema for `opensip-cli.config.yml`. The schema covers `globalExcludes`, `targets`, `checkOverrides`, current fitness/simulation configuration (`failOnErrors`, `maxParallel`, `disabledChecks`, recipe defaults), and the `cli` defaults block. Scheduling keys are not part of the current schema; see the configuration reference roadmap notes for the strict rejection behavior.
...
[`packages/core/src/lib/paths.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/packages/core/src/lib/paths.ts).
...
and [ADR-0155](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/docs/decisions/ADR-0155-canonical-audit-command.md).
...
[ADR-0156](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/docs/decisions/ADR-0156-bounded-stored-impact-proof.md).
...
[ADR-0160](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/docs/decisions/ADR-0160-deterministic-task-context-evidence-plane.md).
...
- **Selector** — the discriminated-union type a recipe uses to pick checks (`all | tags | pattern | explicit`). Lives in [`packages/fitness/engine/src/recipes/types.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.2/packages/fitness/engine/src/recipes/types.ts).
