...
Qualified support is narrower than the Node engine range: it names an exact host tuple (OS, architecture, Node ABI, npm major, and filesystem) with measured evidence. The generated [supported-platforms matrix](/docs/opensip-cli/70-reference/17-supported-platforms/) is authoritative for each host's current status. Unlisted hosts are unqualified — they may work, with no promise, and the CLI never blocks them.
...
1. **[`./02-show-me-the-loops.md`](/docs/opensip-cli/00-start/02-show-me-the-loops/)** — One code sample per tool: a fit check, a sim scenario, a graph rule. See what authoring looks like, now that you know the platform works.
2. **[`./01-what-is-opensip-cli.md`](/docs/opensip-cli/00-start/01-what-is-opensip-cli/)** — The product, the problem, the philosophy. What you just ran, conceptually.
3. **[`../60-guides/00-initialize-your-first-repo.md`](/docs/opensip-cli/60-guides/00-initialize-your-first-repo/)** — The careful repo-adoption version of this page.
4. **[`./05-vocabulary.md`](/docs/opensip-cli/00-start/05-vocabulary/)** — The terms used everywhere: *Tool, recipe, check, scenario, signaler, target, language adapter, plugin, session.*
5. **[`./06-system-context.md`](/docs/opensip-cli/00-start/06-system-context/)** — Where the binary sits between you, the codebase, CI, and OpenSIP Cloud.

After this section, the mental-model section ([`../10-concepts/`](/docs/opensip-cli/10-concepts/)) takes you deep — starting with [`01-fitness-loop.md`](/docs/opensip-cli/10-concepts/01-fitness-loop/), which threads one check end-to-end through the system you just ran.

Once you've internalized the fitness loop, the tool-specific sections — [`../20-fit/`](/docs/opensip-cli/20-fit/), [`../30-sim/`](/docs/opensip-cli/30-sim/), and [`../40-graph/`](/docs/opensip-cli/40-graph/) — go deep on each first-party tool's pipeline, primitives, and gating model.
