...
opensip sim recognizes two kinds, each with its own author-facing entry point in [`packages/simulation/engine/src/index.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.6.0/packages/simulation/engine/src/index.ts):
...
Each kind has its own `define.ts`, `executor.ts`, and `result.ts` under [`packages/simulation/engine/src/kinds/<kind>/`](https://github.com/opensip-ai/opensip-cli/blob/v0.6.0/packages/simulation/engine/src/kinds/). They share a common runtime contract (`RunnableScenario`, `ScenarioExecutorResult`) so the engine can execute any kind through the same dispatcher.
...
A sim recipe is the same shape as a fit recipe: a named selection of scenarios + execution options + reporting options. Defined in [`packages/simulation/engine/src/recipes/types.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.6.0/packages/simulation/engine/src/recipes/types.ts) and constructed via [`defineSimulationRecipe`](https://github.com/opensip-ai/opensip-cli/blob/v0.6.0/packages/simulation/engine/src/recipes/define-recipe.ts).
...
2. **Project-local.** `.js`/`.mjs` files recursively under `<project>/opensip-cli/sim/scenarios/`. Loaded by the plugin discoverer at startup.
3. **npm-package.** Sim packs are discovered from project `node_modules` by **name-pattern** (ADR-0029): any package whose name matches `<scope>/scenarios-*` under the default `@opensip-cli` scope plus any configured `plugins.packageScopes`. There is no `opensipTools.kind: "sim-pack"` marker — sim marker discovery was retired in ADR-0029. Explicit `plugins.scenarioPackages:` pins can additionally name exact packages outside the pattern. For deterministic install/sync, `opensip sim plugin add <pkg>` installs into `.runtime/plugins/sim/` and records the package under `plugins.sim:` (the domain is bound from the `sim` primary — no `--domain` flag). The package's main entry exports `scenarios: RunnableScenario[]` and optionally `recipes: SimulationRecipe[]`.

The registry ([`packages/simulation/engine/src/framework/registry.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.6.0/packages/simulation/engine/src/framework/registry.ts)) is last-writer-wins on id collision. Discovery surfaces conflicts in the CLI's startup logs.
