...
A Tool is a TypeScript object. Five fields, two methods. The whole interface lives at [`packages/core/src/tools/types.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.0/packages/core/src/tools/types.ts):
...
The flow lives in [`packages/cli/src/index.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.0/packages/cli/src/index.ts) and runs once, at process startup, before argv is parsed.
...
1. **A stable kernel.** `@opensip-tools/core` does not import any tool. The layer policy ([dependency-cruiser config](https://github.com/opensip-ai/opensip-tools/blob/v2.3.0/.dependency-cruiser.cjs)) enforces this — `core-imports-nothing-workspace` would fail the build if `core` ever reached up. This means kernel changes are safe: a kernel bump can't break a tool, because the kernel can't see the tool.
