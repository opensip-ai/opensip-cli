...
The layer model the dependency-cruiser config enforces ([`.config/dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.1/.config/dependency-cruiser.cjs)):
...
- **`@opensip-cli/shared-analysis`** is the cross-tool analysis **runtime** extracted from `contracts` ([ADR-0172](https://github.com/opensip-ai/opensip-cli/blob/v0.8.1/docs/decisions/ADR-0172-shared-analysis-layer-extraction.md)): the changed→impact compute engine behind `graph impact` and `fit --changed` / `--include-impacted`, review-brief derivation and correlation, and agent-catalog assembly. It depends on `core` and `contracts` only; tool engines and the host depend on **it**, never the reverse (the `shared-analysis-no-tool-or-cli-edges` dep-cruiser rule). The persisted contract shapes those functions produce (the `ReviewBrief*` and `AgentCatalog*` types, zod schemas, and version constants) stay in `contracts`, which remains a genuine type/constant/facade surface.
...
The layer rule — "dependencies flow up only" — is enforced by [dependency-cruiser](https://github.com/opensip-ai/opensip-cli/blob/v0.8.1/.config/dependency-cruiser.cjs) at lint time. The relevant rules:
...
Real codebases have edge cases. Two earlier cross-layer exceptions once lived in [`.config/dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-cli/blob/v0.8.1/.config/dependency-cruiser.cjs); both have since been **paid down** and deleted:
...
Two decisions ([ADR-0151](https://github.com/opensip-ai/opensip-cli/blob/v0.8.1/docs/decisions/ADR-0151-manifest-derived-package-and-export-boundaries.md)
and [ADR-0150](https://github.com/opensip-ai/opensip-cli/blob/v0.8.1/docs/decisions/ADR-0150-production-builds-publish-runtime-artifacts-only.md))
