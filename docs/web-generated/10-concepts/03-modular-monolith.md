...
purpose: "The 56-package monorepo, the six-layer dependency rule, why dependency-cruiser exists, and the trade-offs."
source-files:
...
> - Why opensip-cli ships as 56 workspace packages instead of one.
...
The layer model the dependency-cruiser config enforces ([`.config/dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-cli/blob/v0.5.1/.config/dependency-cruiser.cjs)):
...
The layer rule — "dependencies flow up only" — is enforced by [dependency-cruiser](https://github.com/opensip-ai/opensip-cli/blob/v0.5.1/.config/dependency-cruiser.cjs) at lint time. The relevant rules:
...
Real codebases have edge cases. Two earlier cross-layer exceptions once lived in [`.config/dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-cli/blob/v0.5.1/.config/dependency-cruiser.cjs); both have since been **paid down** and deleted:
...
## Why 56 workspace packages and not 1
...
…and not pull in the JavaScript universe. With a single mega-package, every install pulls every check. With 56 workspace packages, an install pulls only what's needed. (Today the bundled distribution still installs everything; tomorrow's tree-shaken or selectively-installed distribution doesn't have to.)
...
A flat package can have any internal structure. With 56 workspace packages, the layer is the directory structure: looking at `packages/` tells you the architecture in five seconds. If a contributor accidentally adds an upward edge, the build fails before the PR is even reviewed. The layer rule isn't aspiration — it's a wall.
...
- **[`04-contract-surfaces.md`](/docs/opensip-cli/10-concepts/04-contract-surfaces/)** — the public edges this layer cake exposes. The Tool contract sits at the top of Layer 3; the JSON output sits across Layer 2.
- **[`../70-reference/02-package-catalog.md`](/docs/opensip-cli/70-reference/02-package-catalog/)** — every package, by layer, with one-line role and key exports. Use this when you're hunting for a symbol.
- **[`../80-implementation/05-layer-policy.md`](/docs/opensip-cli/80-implementation/05-layer-policy/)** — the dep-cruiser config, rule by rule, with rationale.
