...
The layer model the dependency-cruiser config enforces ([`.config/dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-cli/blob/v1.0.0/.config/dependency-cruiser.cjs)):
...
- **`@opensip-cli/dashboard`** is the self-contained HTML report renderer; consumed by the CLI-owned `dashboard` command and each tool's auto-open hook. It does not implement the `Tool` contract; it is a library the composition root consumes.
...
The layer rule — "dependencies flow up only" — is enforced by [dependency-cruiser](https://github.com/opensip-ai/opensip-cli/blob/v1.0.0/.config/dependency-cruiser.cjs) at lint time. The relevant rules:
...
Real codebases have edge cases. Two earlier cross-layer exceptions once lived in [`.config/dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-cli/blob/v1.0.0/.config/dependency-cruiser.cjs); both have since been **paid down** and deleted:
