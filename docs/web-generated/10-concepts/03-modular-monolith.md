...
The layer rule — "dependencies flow up only" — is enforced by [dependency-cruiser](https://github.com/opensip-ai/opensip-tools/blob/v2.7.1/.config/dependency-cruiser.cjs) at lint time. The relevant rules:
...
Real codebases have edge cases. Two earlier cross-layer exceptions once lived in [`.config/dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-tools/blob/v2.7.1/.config/dependency-cruiser.cjs); both have since been **paid down** and deleted:
