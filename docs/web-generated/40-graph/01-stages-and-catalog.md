...
Stages 0–2 are language-agnostic over the [`GraphLanguageAdapter`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/lang-adapter/types.ts) contract. The orchestrator looks up an adapter via [`lang-adapter/registry.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/lang-adapter/registry.ts), then dispatches `discoverFiles`, `parseProject`, `walkProject`, and `resolveCallSites` through it. Stage 3 (`buildIndexes`) lives in [`packages/graph/engine/src/pipeline/indexes.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/pipeline/indexes.ts); stages 4–5 live in [`packages/graph/engine/src/rules/`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/rules/) and [`packages/graph/engine/src/render/`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/render/).
...
> **Adapter layer.** Five first-party adapters ship in v2.0.0, each as its own publishable npm package under the `@opensip-tools/graph-*` namespace: TypeScript ([`@opensip-tools/graph-typescript`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-typescript/src/index.ts)), Python ([`@opensip-tools/graph-python`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-python/src/index.ts)), Rust ([`@opensip-tools/graph-rust`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-rust/src/index.ts)), Go ([`@opensip-tools/graph-go`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-go/src/index.ts)), and Java ([`@opensip-tools/graph-java`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-java/src/index.ts)). Discovery is by **name pattern**: the CLI walks `node_modules` for any package whose name matches `@opensip-tools/graph-*` (see [`discoverGraphAdapterPackages`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/plugins/graph-adapter-discovery.ts)) — or, if `plugins.graphAdapters` is set in `opensip-tools.config.yml`, exactly that explicit list. The CLI bootstrap path ([`register-graph-adapters.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/cli/src/bootstrap/register-graph-adapters.ts)) imports each discovered package and registers its `adapter` export. `pickAdapter(cwd)` chooses by file-extension dominance with a deterministic preference order on ties. Stages 3, 4, and 5 consume the catalog without knowing which adapter built it.

### Stage 0 — Discover

`adapter.discoverFiles({ cwd, configPathOverride? })` produces a sorted, deduplicated list of absolute file paths. The adapter chooses how — the TypeScript adapter ([`graph-typescript/discover.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-typescript/src/discover.ts)) resolves `tsconfig.json` and applies its `include` / `exclude`; the Python adapter ([`graph-python/discover.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-python/src/discover.ts)) reads `pyproject.toml` / `setup.py` and falls back to a `**/*.py` glob; the Rust adapter ([`graph-rust/discover.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-rust/src/discover.ts)) reads `Cargo.toml` and falls back to `**/*.rs`; the Go and Java adapters apply analogous module-file + glob fallbacks. No parser state is created here — that's stage 1's job. Stage 0 is purely about *what files exist*.
...
For the TypeScript adapter, resolver logic is split into one file per call shape in [`graph-typescript/edge-resolvers/`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-typescript/src/edge-resolvers/): direct calls, property-access calls, JSX elements, `new` expressions, polymorphic dispatch, and a catalog-fallback resolver that handles the long tail. The tree-sitter adapters take the simpler approach — a single `resolve.ts` per adapter that does name-based lookup against the frozen catalog ([`graph-python/resolve.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-python/src/resolve.ts), [`graph-rust/resolve.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/graph-rust/src/resolve.ts), and equivalents for graph-go and graph-java).
...
[`pipeline/indexes.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/pipeline/indexes.ts) performs a linear scan over the now-complete catalog and builds inverted indexes that rules need:
...
The CLI handler [`cli/graph.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/cli/graph.ts) picks the renderer based on flags and writes its output.
...
[`cache/invalidate.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.3.2/packages/graph/engine/src/cache/invalidate.ts) classifies a cached catalog into one of three verdicts:
