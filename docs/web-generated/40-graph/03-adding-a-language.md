...
**Third-party graph adapters** are supported via the same `opensipTools.kind: "graph-adapter"` marker the first-party packages use. The marker walker (see [`80-implementation/02-plugin-loader.md`](/docs/opensip-tools/80-implementation/02-plugin-loader/)) discovers any installed package declaring that kind and registers its `adapter` export. The adapter contract types (`GraphLanguageAdapter`, `registerAdapter`, `pickAdapter`) are exported from `@opensip-tools/graph`.
