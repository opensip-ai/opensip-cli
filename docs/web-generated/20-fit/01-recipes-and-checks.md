...
A check is the result of `defineCheck()`. The configuration shape is in [`packages/fitness/engine/src/framework/check-config.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.10/packages/fitness/engine/src/framework/check-config.ts); the validator is Zod-driven, so misconfigured checks throw at definition time, not at run time.
...
| Field | What it does |
|---|---|
| `scope: { languages, concerns }` | Marketplace-ready target matching. See [`02-targets-and-scope.md`](/docs/opensip-cli/20-fit/02-targets-and-scope/). |
...
A `Check` object ([`packages/fitness/engine/src/framework/check-types.ts:45`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.10/packages/fitness/engine/src/framework/check-types.ts)):
...
The full input shape is in [`packages/fitness/engine/src/recipes/types.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.10/packages/fitness/engine/src/recipes/types.ts):
...
Inside the check, read your slice via `getCheckConfig<T>(slug)` ([`packages/fitness/engine/src/recipes/check-config.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.10/packages/fitness/engine/src/recipes/check-config.ts)):
...
- **[`02-targets-and-scope.md`](/docs/opensip-cli/20-fit/02-targets-and-scope/)** — how the framework decides which files a check runs against. Where `scope`, `fileTypes`, and the targets registry interact.
- **[`03-ignore-directives.md`](/docs/opensip-cli/20-fit/03-ignore-directives/)** — inline source-level suppression for individual violations.
- **[`../50-extend/01-plugin-authoring.md`](/docs/opensip-cli/50-extend/01-plugin-authoring/)** — full walkthrough of authoring a check pack, a project-local check, and a custom recipe.
