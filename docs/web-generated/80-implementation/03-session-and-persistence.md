...
Source of truth: [`packages/core/src/lib/paths.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/packages/core/src/lib/paths.ts). Every consumer reads paths through `resolveProjectPaths(cwd)`. The directory is created lazily by whichever consumer needs a subpath first; `mkdirSync(..., { recursive: true })` is the standard idiom.
...
[`packages/datastore`](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/packages/datastore) hosts the persistence kernel: a `DataStore` interface, a SQLite-backed implementation, an in-memory implementation for tests, and the workspace-wide migration store under `migrations/`. The CLI bootstrap opens one `DataStore` per invocation in the `preAction` hook ([`packages/cli/src/index.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/packages/cli/src/index.ts)) and closes it on `process.exit`. Tool commands do not receive a raw datastore handle on `ToolCliContext`; they use the entered `RunScope` for read-owned internals and the host-owned seams (`toolState`, baseline/export seams, `writeArtifact`) for durable writes.
...
([ADR-0146](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/docs/decisions/ADR-0146-host-plane-reserved-state-namespace.md)).
...
`decodeSessionPayload` in [`@opensip-cli/session-store`](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/packages/session-store/src/session-payload-decode.ts)
...
([ADR-0117](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/docs/decisions/ADR-0117-host-owned-analysis-run-pipeline.md)).
...
See [ADR-0051](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/docs/decisions/ADR-0051-host-owned-run-lifecycle-timing.md) and the cross-cutting contracts in the host-owned-run-timing plan for the full seam, logging, and hardening details.
...
"unknown", not "defaults". See [ADR-0097](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/docs/decisions/ADR-0097-gate-verdict-determinism.md).
...
The logger is in [`packages/core/src/lib/logger.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.5.0/packages/core/src/lib/logger.ts). Every log entry carries:
