...
the shared `decodeSessionPayload` in [`@opensip-cli/session-store`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.2/packages/session-store/src/session-payload-decode.ts)
...
See [ADR-0051](https://github.com/opensip-ai/opensip-cli/blob/v0.1.2/docs/decisions/ADR-0051-host-owned-run-lifecycle-timing.md) and the cross-cutting contracts in the host-owned-run-timing plan for the full seam, logging, and hardening details.
...
The logger is in [`packages/core/src/lib/logger.ts`](https://github.com/opensip-ai/opensip-cli/blob/v0.1.2/packages/core/src/lib/logger.ts). Every log entry carries:
