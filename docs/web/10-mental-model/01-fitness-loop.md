...
2. **The project config.** Read from `<project>/opensip-tools.config.yml` (or the path passed via `--config`). The config carries `targets:`, `plugins:`, `globalExcludes:`, recipe overrides, and reporting defaults. See [`80-reference/02-configuration.md`](/docs/opensip-tools/80-reference/02-configuration/) for the full schema.
