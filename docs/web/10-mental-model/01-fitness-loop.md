...
related-docs:
  - ../00-orientation/01-what-is-opensip-tools.md
...
We trace one specific scenario: a single TypeScript check named `no-console-log` running inside `acme-api`. We follow it from the moment you type `opensip-tools fit` to the moment your shell prompt reappears.
...
Source: [`packages/cli/src/index.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/cli/src/index.ts)

`opensip-tools fit` invokes the binary. The CLI's job at this stage is small:

1. Parse global flags (`--debug`, `--quiet`).
2. Set up the logger and assign a `runId` (`run_<base32>`).
...
4. Hand argv to Commander, which dispatches to `fitnessTool`'s `fit` action handler ([`packages/fitness/engine/src/tool.ts:118`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/tool.ts)).
...
> **Where the example lands:** the binary is `opensip-tools`, argv is `['fit']` (defaults applied), the resolved Tool is `fitnessTool` with metadata `{ id: 'fitness', version: '1.0.0' }`.

---

## Stage 2 — Config and paths

Source: [`packages/core/src/lib/paths.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/lib/paths.ts), the configure command for user-level settings, and `loadProjectConfig()` inside `executeFit()`.
...
2. **The project config.** Read from `<project>/opensip-tools.config.yml` (or the path passed via `--config`). The config carries `targets:`, `plugins:`, `globalExcludes:`, recipe overrides, and reporting defaults. See [`70-reference/02-configuration.md`](/docs/opensip-tools/70-reference/02-configuration/) for the full schema.
...
Source: [`packages/core/src/plugins/discover.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/plugins/discover.ts), [`packages/fitness/engine/src/plugins/`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/plugins/), [`packages/cli/src/index.ts:64-69`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/cli/src/index.ts) (language-adapter registration).
...
> **Where the example lands:** `acme-api` ends up with the universal pack, the typescript pack, the python pack, and three custom checks (`require-cdk-json-exists`, `no-print-outside-pipelines`, `infra-tag-required`). The `no-console-log` check we're tracing comes from `@opensip-tools/checks-typescript`.

---

## Stage 4 — Recipe selection

Source: [`packages/fitness/engine/src/recipes/registry.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/recipes/registry.ts), [`packages/fitness/engine/src/recipes/built-in-recipes.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/recipes/built-in-recipes.ts).
...
Source: [`packages/fitness/engine/src/targets/resolver.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/targets/resolver.ts), [`packages/fitness/engine/src/framework/path-matcher.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/framework/path-matcher.ts), [`packages/fitness/engine/src/framework/scope-resolver.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/framework/scope-resolver.ts).
...
Source: [`packages/fitness/engine/src/framework/define-check.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/framework/define-check.ts), [`packages/fitness/engine/src/recipes/parallel-execution.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/recipes/parallel-execution.ts), [`packages/fitness/engine/src/recipes/sequential-execution.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/recipes/sequential-execution.ts).
...
- `signal` — an `AbortSignal` honored by long-running checks.
- `readFile(path)` — cached, abort-aware file read.
- `matchFiles()` — the resolved file list from Stage 5.
- `log(msg)` — per-check logger sink.
...
Source: [`packages/core/src/types/signal.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/types/signal.ts), [`packages/contracts/src/types.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/contracts/src/types.ts), [`packages/fitness/engine/src/framework/result-builder.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/framework/result-builder.ts).
...
The `CliOutput` type ([`packages/contracts/src/types.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/contracts/src/types.ts)) is the canonical envelope: version, tool, timestamp, score, passed flag, summary, durationMs, and a `checks[]` array each with its own findings. Anything that consumes the JSON output (CI, dashboard, the gate) reads `CliOutput`.
...
Source: [`packages/cli/src/ui/`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/cli/src/ui/), [`packages/fitness/engine/src/cli/fit.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/fitness/engine/src/cli/fit.ts), [`packages/cli/src/open-dashboard.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/cli/src/open-dashboard.ts).
...
After rendering, the dashboard auto-open runs if conditions allow: `--open` was passed (or the user opted into auto-open in their config), output isn't `--json`, and stdout is a TTY. The HTML report at `<project>/opensip-tools/.runtime/reports/<run-id>/index.html` opens in the user's default browser.
...
- **[`02-tool-plugin-model.md`](/docs/opensip-tools/10-mental-model/02-tool-plugin-model/)** — how the CLI doesn't know what `fit` does. Stage 1 in depth.
- **[`03-modular-monolith.md`](/docs/opensip-tools/10-mental-model/03-modular-monolith/)** — the 18-package layer cake that makes Stages 1, 3, and 6 isolatable.
- **[`04-contract-surfaces.md`](/docs/opensip-tools/10-mental-model/04-contract-surfaces/)** — the public edges: argv, Tool interface, plugin manifest, CliOutput.

When you want stage-by-stage detail, jump to [`../20-the-fit-loop/`](/docs/opensip-tools/20-the-fit-loop/) — each doc there expands one of these stages with full code paths.
