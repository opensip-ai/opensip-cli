...
The `collectChecks` helper in [`packages/fitness/checks-universal/src/index.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/fitness/checks-universal/src/index.ts) walks the package's own subdirectory exports and flattens them, deduplicating by check id. New checks are added by writing a new file under `src/checks/<category>/` — the barrel export picks them up automatically.
...
| Pack | Path | Scope |
|---|---|---|
| `@opensip-tools/checks-universal` | `packages/fitness/checks-universal/` | Cross-language checks (text/regex/file shape), e.g. file-length, TODO scanners, security secret detection. |
| `@opensip-tools/checks-typescript` | `packages/fitness/checks-typescript/` | TypeScript-specific: complex-function via AST, dead-code detection, dependency rules, react/hook patterns. |
| `@opensip-tools/checks-python` | `packages/fitness/checks-python/` | Python-specific: print() detection, docstring requirements, mutable default arguments. |
| `@opensip-tools/checks-java` | `packages/fitness/checks-java/` | Java-specific patterns. |
| `@opensip-tools/checks-go` | `packages/fitness/checks-go/` | Go-specific patterns. |
| `@opensip-tools/checks-cpp` | `packages/fitness/checks-cpp/` | C/C++-specific patterns. |
...
The `CHECK_DISPLAY` map ([`packages/fitness/checks-universal/src/display/index.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/fitness/checks-universal/src/display/index.ts) and analogues) maps a check slug to `[icon, displayName]`:
...
For the full walkthrough — boilerplate, testing, publishing — see [`60-surfaces/02-plugin-authoring.md`](/docs/opensip-tools/60-surfaces/02-plugin-authoring/).
...
- `@opensip-tools/checks-python` — no-mutable-defaults, docstring-required.
- Project-local `<project>/opensip-tools/fit/checks/` — three custom checks.

The dashboard groups by category (universal pack's display map provides the icons), shows pack-of-origin in the verbose view, and highlights checks with project-level overrides. The CLI's `fit-list` command shows the full inventory: ~80 checks, source-tagged.

---

## What's next

- **[`03-architecture-gate.md`](/docs/opensip-tools/50-subsystems/03-architecture-gate/)** — the regression-detection workflow built on top of check output.
- **[`../60-surfaces/02-plugin-authoring.md`](/docs/opensip-tools/60-surfaces/02-plugin-authoring/)** — full walkthrough of authoring a pack from scratch.
- **[`../70-reference/01-package-catalog.md`](/docs/opensip-tools/70-reference/01-package-catalog/)** — every pack's package, key exports, layer.
