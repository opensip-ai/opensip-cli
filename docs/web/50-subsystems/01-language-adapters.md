...
[`packages/core/src/languages/adapter.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/languages/adapter.ts):
...
The framework's content-filter dispatcher ([`packages/core/src/languages/content-filter-dispatch.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/languages/content-filter-dispatch.ts)) maps the check's `contentFilter` setting to one of these:
...
Called by the CLI bootstrap before any check runs. Used by adapters that need async initialization — tree-sitter WASM modules need to be instantiated, for instance. Adapters that don't need warmup leave the field undefined.

The CLI awaits `Promise.all(adapter.warmup?.() ...)` if any adapter declares warmup. A warmup that fails is logged and the adapter is dropped from the registry; the rest continue.

---

## The registry

[`packages/core/src/languages/registry.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/languages/registry.ts) holds the in-memory list. Registration is by id; dispatch is by extension:
...
defaultLanguageRegistry.byExtension('.ts');         // → typescriptAdapter
defaultLanguageRegistry.byId('rust');               // → rustAdapter
defaultLanguageRegistry.byScopeLanguage('rust');    // checks 'aliases' too
```

The CLI registers all six bundled adapters at module load time before any Tool runs. See [`packages/cli/src/index.ts:64-69`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/cli/src/index.ts).
...
| `lang-python` | `packages/languages/lang-python/` | `.py` | Hand-written strip routines |
...
| `lang-cpp` | `packages/languages/lang-cpp/` | `.c`, `.cc`, `.cpp`, `.cxx`, `.h`, `.hpp` | Hand-written strip routines |
...
- **[`02-check-packs.md`](/docs/opensip-tools/50-subsystems/02-check-packs/)** — how check packs build on adapters: scope filters, parameterization, the marketplace shape.
- **[`../70-reference/01-package-catalog.md`](/docs/opensip-tools/70-reference/01-package-catalog/)** — every adapter package, key exports, where to find them.
