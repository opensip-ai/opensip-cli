...
├── sessions/<run-id>.json         ← per-run records (durable)
├── reports/<run-id>/              ← dashboard HTML (durable until purged)
│   ├── index.html
│   └── …assets…
├── logs/<run-id>.jsonl            ← structured log lines (durable until purged)
...
Source of truth: [`packages/core/src/lib/paths.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/core/src/lib/paths.ts). Every consumer reads paths through `resolveProjectPaths(cwd)`.
...
The shape lives in [`packages/contracts/src/persistence/store.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/contracts/src/persistence/store.ts):
...
```bash
opensip-tools sessions list                      # list past runs
opensip-tools sessions list --tool fit            # filter by tool
opensip-tools sessions list --limit 50            # most recent 50
opensip-tools sessions purge                      # delete all (prompts for confirm)
opensip-tools sessions purge --keep 10            # keep most recent 10
```

The list command sorts by timestamp descending and shows id, tool, recipe, pass/fail, and finding count. The dashboard reads the same store to populate its run history.
...
The session count grows over time; `sessions purge --keep N` is the periodic-cleanup answer. Some teams add a cron-style cleanup; others just let `.runtime/` accumulate and wipe periodically. The dir is gitignored, so it doesn't grow the repo.
...
The logger is in [`packages/core/src/lib/logger.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/core/src/lib/logger.ts). Every log entry carries:
...
The dashboard auto-open hook is wired into the Tool action handler. After a run, if (a) `--open` was requested or auto-open is configured, (b) output isn't `--json`, and (c) stdout is a TTY, the CLI launches the user's default browser onto the report URL. Logic in [`packages/cli/src/open-dashboard.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/cli/src/open-dashboard.ts).
...
Source: [`packages/fitness/engine/src/framework/parse-cache.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/fitness/engine/src/framework/parse-cache.ts).
...
Source: [`packages/fitness/engine/src/framework/file-cache.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.5/packages/fitness/engine/src/framework/file-cache.ts).
...
|---|---|---|
| `sessions/<id>.json` | yes | History entry disappears. |
| `logs/<id>.jsonl` | yes | Log archive disappears. |
| `reports/<id>/` | yes | Report disappears; dashboard for that run errors. |
...
- **[`../50-subsystems/03-architecture-gate.md`](/docs/opensip-tools/50-subsystems/03-architecture-gate/)** — the gate's full behavior and the baseline format.
- **[`../60-surfaces/03-dashboard.md`](/docs/opensip-tools/60-surfaces/03-dashboard/)** — the HTML report's structure and the `dashboard` command.
- **[`../70-reference/02-configuration.md`](/docs/opensip-tools/70-reference/02-configuration/)** — `opensip-tools.config.yml` schema (the one bit of project state that's not in `.runtime/`).
