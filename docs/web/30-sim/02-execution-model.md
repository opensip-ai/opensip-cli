...
The chaos executor delegates the tick loop to the same shared `runLoadWindow` driver the load kind uses ([`packages/simulation/engine/src/framework/execution/run-load-window.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/simulation/engine/src/framework/execution/run-load-window.ts)) and supplies an `injectChaos` callback per the Template Method pattern. The full sequence:
...
- **[`../80-internals/`](/docs/opensip-tools/80-internals/)** — how the CLI dispatches both fit and sim. Plugin loading, session writing, persistence layout.
