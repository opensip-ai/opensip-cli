...
opensip-tools is a CLI that runs against your project. This doc draws the box around it: who calls it, what it reads and writes, what's local versus cross-project, what's optional.

> **What you'll understand after this:**
> - The four actors that interact with opensip-tools (you, CI, the dashboard browser, the cloud).
...
The layout is set by [`packages/core/src/lib/paths.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/core/src/lib/paths.ts) and is the single source of truth for every consumer (logger, gate, plugin loader, dashboard, sessions store).
...
```
<project>/
├── opensip-tools.config.yml       ← project config (commit this)
└── opensip-tools/
    ├── fit/
...
Gitignored (`opensip-tools init` adds the entry to `.gitignore` for you):
...
The user-level dir is intentionally small. Anything project-specific (checks, recipes, plugins) lives in the project, not here. The `~/.opensip-tools/` dir exists so every project on your machine can share one cloud API key.

`opensip-tools configure` is the command that creates and edits this file. `opensip-tools uninstall` deletes the whole `~/.opensip-tools/` directory.

---

## Exit codes

opensip-tools follows the conventional Unix exit-code shape, defined in [`packages/contracts/src/exit-codes.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/contracts/src/exit-codes.ts):
...
We'll see how each layer of the system handles this project — from the CLI dispatch in [`../80-internals/01-cli-dispatch.md`](/docs/opensip-tools/80-internals/01-cli-dispatch/), to the language-adapter registration, to the recipe selection, to the gate diff. Every runtime doc has a "Where the example lands" section so you can trace the same scenario all the way down.

---

## What's next

Orientation done. The mental-model section ([`../10-concepts/`](/docs/opensip-tools/10-concepts/)) is where the architecture starts to land. Open with [`01-fitness-loop.md`](/docs/opensip-tools/10-concepts/01-fitness-loop/) — it's the spine of the doc set.
