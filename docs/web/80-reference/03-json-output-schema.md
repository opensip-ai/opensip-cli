...
related-docs:
  ...
  - ../30-the-sim-loop/02-execution-model.md
---
# JSON output schema
...
The shapes live in [`packages/contracts/src/types.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.10.0/packages/contracts/src/types.ts).
...
Each rule appears as a `CheckOutput` whose `checkSlug` is the graph rule slug (`graph:orphan-subtree`, `graph:duplicated-function-body`, `graph:no-side-effect-path`, `graph:test-only-reachable`, `graph:always-throws-branch`). Findings carry the same `FindingOutput` shape; graph attaches a `partialFingerprints.bodyHash` in the SARIF output but the JSON envelope keeps the standard fields. See the renderer at [`packages/graph/engine/src/render/json.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.10.0/packages/graph/engine/src/render/json.ts).
...
For SARIF (the gate's native shape), use `--gate-save` / `--gate-compare`. The SARIF shape is the SARIF 2.1.0 spec's, not opensip-tools' — see [`60-subsystems/03-architecture-gate.md`](/docs/opensip-tools/60-subsystems/03-architecture-gate/).

---

## What's next

- **[`02-configuration.md`](/docs/opensip-tools/80-reference/02-configuration/)** — `opensip-tools.config.yml` schema (the *input* shape).
- **[`../10-mental-model/04-contract-surfaces.md`](/docs/opensip-tools/10-mental-model/04-contract-surfaces/)** — every contract surface, with stability tiers.
