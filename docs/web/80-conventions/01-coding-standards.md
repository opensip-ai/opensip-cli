...
`tsconfig.json` extends `@tsconfig/node20`. Every package has its own `tsconfig.json` with `composite: true` for project references. The workspace root tsconfig pulls them all in.

Notable settings:

- `strict: true` — all strict-mode flags on.
- `noImplicitOverride: true` — explicit `override` keyword required.
- `noFallthroughCasesInSwitch: true` — every `case` ends in `break`/`return`/`throw`.
- `noUncheckedIndexedAccess: true` — array/object access produces `T | undefined`. The codebase handles undefined explicitly; no `arr[i]!`.
- `noPropertyAccessFromIndexSignature: true` — bracket access for index signatures, dot access for declared properties.
...
Flat config at [`eslint.config.mjs`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/eslint.config.mjs). The base layers:
...
[`packages/core/src/lib/errors.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/lib/errors.ts) defines the workspace's error hierarchy:
...
Codes are mapped to user-facing suggestions in [`packages/contracts/src/exit-codes.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/contracts/src/exit-codes.ts):

```ts
export function getErrorSuggestion(code: string): string | undefined {
  switch (code) {
    case 'CONFIG.MISSING':         return 'Run `opensip-tools init` to create one.';
    case 'GATE.BASELINE.NOT_FOUND': return 'Run with `--gate-save` first to create a baseline.';
    // ...
  }
}
```

The CLI calls `getErrorSuggestion(error.code)` and threads the result into the `ErrorResult` that the renderer shows. Tools throw with a code; the CLI handles the rendering.

---

## Exit codes

Defined exactly once in [`packages/contracts/src/exit-codes.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/contracts/src/exit-codes.ts):

```ts
export const EXIT_CODES = {
  SUCCESS: 0,
  RUNTIME_ERROR: 1,           // checks/scenarios failed
  CONFIGURATION_ERROR: 2,     // run could not start
} as const;
```
...
Adding a new exit code is a major-version change — see [`10-mental-model/04-contract-surfaces.md`](/docs/opensip-tools/10-mental-model/04-contract-surfaces/).

---

## Logging

The structured logger is in [`packages/core/src/lib/logger.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.0.10/packages/core/src/lib/logger.ts). Every log entry carries:
...
Type-only imports use `import type` so they're erased at compile time. The dep-cruiser config explicitly ignores type-only imports (`tsPreCompilationDeps: false`), so a module can `import type` from a higher layer without violating the layer rule.
...
- **[`02-layer-policy.md`](/docs/opensip-tools/80-conventions/02-layer-policy/)** — the dep-cruiser config, rule by rule, with rationale.
- **[`03-doc-conventions.md`](/docs/opensip-tools/80-conventions/03-doc-conventions/)** — voice, frontmatter, and verification trails for documentation.
- **[`../70-reference/01-package-catalog.md`](/docs/opensip-tools/70-reference/01-package-catalog/)** — the workspace package list these standards apply to.
