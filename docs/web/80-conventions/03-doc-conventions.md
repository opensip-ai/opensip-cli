...
- **Assumes engineering fluency.** Don't re-explain `tsconfig`, `npm`, `glob pattern`, `ESM module`, or `JSON Lines`. Do explain opensip-tools-specific terms (Tool, Check, Recipe, Target) the first time they appear in a doc, with a link to [`../00-orientation/02-vocabulary.md`](/docs/opensip-tools/00-orientation/02-vocabulary/).
...
related-docs:
  - ../00-orientation/02-vocabulary.md  # cross-links into the doc set
---
```
...
Use `[**Title**](/docs/opensip-tools/80-conventions/path/)` for prominent cross-links and `[`identifier`](/docs/opensip-tools/80-conventions/path/)` for in-line code references.
...
- `packages/` directory listing (17 packages …).
...
Most contributions don't need a new doc. The existing 28 cover the architecture; new shape is rare.
...
Reference (70) and conventions (80) docs are exceptions — those grow with the system, and a new entry typically slots in as a new section rather than a new doc.
...
Splitting is right when the new content has its own audience or its own reading order — see how the fit loop split into four 20-section docs rather than one giant one.
...
- **[`01-coding-standards.md`](/docs/opensip-tools/80-conventions/01-coding-standards/)** — code conventions (the parallel to this doc, for source).
- **[`02-layer-policy.md`](/docs/opensip-tools/80-conventions/02-layer-policy/)** — the dep-cruiser rules.
