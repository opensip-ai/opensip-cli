...
Source of truth: [`packages/core/src/lib/paths.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.3.0/packages/core/src/lib/paths.ts). Every consumer reads paths through `resolveProjectPaths(cwd)`.
...
The shape lives in [`packages/contracts/src/persistence/store.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.3.0/packages/contracts/src/persistence/store.ts):
...
The logger is in [`packages/core/src/lib/logger.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.3.0/packages/core/src/lib/logger.ts). Every log entry carries:
...
The dashboard auto-open hook is wired into the Tool action handler. After a run, if (a) `--open` was requested or auto-open is configured, (b) output isn't `--json`, and (c) stdout is a TTY, the CLI launches the user's default browser onto the report URL. Logic in [`packages/cli/src/open-dashboard.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.3.0/packages/cli/src/open-dashboard.ts).
...
Source: [`packages/fitness/engine/src/framework/parse-cache.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.3.0/packages/fitness/engine/src/framework/parse-cache.ts).
...
Source: [`packages/fitness/engine/src/framework/file-cache.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.3.0/packages/fitness/engine/src/framework/file-cache.ts).
...
The `clear` command ([`packages/cli/src/commands/clear.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.3.0/packages/cli/src/commands/clear.ts)) only deletes session JSON files under `.runtime/sessions/`. It does not touch reports, logs, cache, plugins, or the baseline — those have to be removed manually if you want them gone.
