...
The layout is set by [`packages/core/src/lib/paths.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.10.0/packages/core/src/lib/paths.ts) and is the single source of truth for every consumer (logger, gate, plugin loader, dashboard, sessions store).
...
opensip-tools follows the conventional Unix exit-code shape, defined in [`packages/contracts/src/exit-codes.ts`](https://github.com/opensip-ai/opensip-tools/blob/v1.10.0/packages/contracts/src/exit-codes.ts):
...
Orientation done. The mental-model section ([`../10-mental-model/`](/docs/opensip-tools/10-mental-model/)) is where the architecture starts to land. Open with [`01-fitness-loop.md`](/docs/opensip-tools/10-mental-model/01-fitness-loop/) — it's the spine of the doc set.
