...
Every rule lives in [`packages/graph/engine/src/rules/<rule-name>.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/graph/engine/src/rules/) and exports a single value implementing this shape:
...
The five rules below are registered in [`rules/registry.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/graph/engine/src/rules/registry.ts) and run on every `graph` invocation unless the caller filters with `--check <slug>` (planned, not yet shipped) or `--no-check <slug>` (also planned).
...
[`rules/orphan-subtree.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/graph/engine/src/rules/orphan-subtree.ts) — find functions not reachable from any inferred entry point.
...
[`rules/test-only-reachable.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/graph/engine/src/rules/test-only-reachable.ts) — compute reachability from the inferred entry points. Any occurrence reachable only via files where `inTestFile` is true is flagged.
...
[`rules/_entry-points.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/graph/engine/src/rules/_entry-points.ts) is consumed by `orphan-subtree` and `test-only-reachable`. It's not itself a rule (note the leading underscore). The current implementation classifies each occurrence into one of three reasons:
...
The fidelity matrix from [plan 10 §6](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/docs/plans/10-graph-language-pluggability.md):
...
The `ruleHints` surface ([`lang-adapter/types.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/graph/engine/src/lang-adapter/types.ts)) is how an adapter customises the per-rule inputs without changing rule logic: `isTestFile` for `test-only-reachable`, `sideEffectPrimitives` for `no-side-effect-path`, `throwSyntaxRegex` for `always-throws-branch`. An adapter that doesn't supply hints gets the engine's defaults and the corresponding rules silently degrade in precision rather than failing.
...
v2: the baseline lives in the project's SQLite store (`<project>/opensip-tools/.runtime/datastore.sqlite`, gitignored), in the `graph_baseline_signals` table. There is exactly one baseline per project; the v1 `--baseline <path>` flag is gone (see [v2.0.0 CHANGELOG](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/CHANGELOG.md)).
...
The SARIF mapping in [`render/sarif.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/graph/engine/src/render/sarif.ts):

| Graph concept | SARIF field |
|---|---|
| Run (per-rule) | `runs[i].tool.driver.name = <rule slug>` (one SARIF run per check/rule; see `buildSarifLog` in [`packages/fitness/engine/src/sarif.ts`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/packages/fitness/engine/src/sarif.ts), which assigns `driver.name = ch.checkSlug`) |
...
- **[`../plans/00-graph-performance-improvements.md`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/docs/plans/00-graph-performance-improvements.md)** — perf-plan history covering the rule and pipeline evolution since v1.1.
