...
For third-party adapters: declare `opensipTools.kind: "graph-adapter"` in your package's `package.json`. The marker walker (see [`80-internals/02-plugin-loader.md`](/docs/opensip-tools/80-internals/02-plugin-loader/)) discovers any installed package declaring that kind and registers it through the same `registerAdapter` seam. The adapter contract types (`GraphLanguageAdapter`, `registerAdapter`, `pickAdapter`) are exported via deep monorepo paths today; promoting them to the public `@opensip-tools/graph` barrel for third-party consumption is on the roadmap.
