...
`@opensip-tools/lang-typescript` re-exports `filterContent`, `clearFilterCache`, and `FilteredContent` from `@opensip-tools/fitness`. This is the documented exception to the "lang packs depend only on core" layer rule — see [`80-internals/05-layer-policy.md`](/docs/opensip-tools/80-internals/05-layer-policy/) and the named `lang-no-fitness-except-typescript` carve-out in [`.dependency-cruiser.cjs`](https://github.com/opensip-ai/opensip-tools/blob/v2.0.0/.dependency-cruiser.cjs).
