...
6. **Catalog read path stays as a JSON read for this phase.** The dashboard generator (`generator.ts` at `packages/contracts/src/persistence/dashboard/`) is pure — it receives a `GraphCatalog | null` as its 4th argument. The actual JSON catalog read lives in the *caller*: `packages/fitness/engine/src/cli/dashboard.ts:loadGraphCatalog` (line 98), which does `existsSync` + `readFileSync` + `JSON.parse` on `paths.graphCatalogPath`. **Phase 1 changes only the session read path; the caller's catalog read keeps calling `loadGraphCatalog` until Phase 3 deletes `cache/read.ts` and the catalog JSON ceases to exist.** Phase 3 Task 3.3 owns swapping `loadGraphCatalog` to `new CatalogRepo(datastore).loadFullCatalog()`. This preserves Phase 1 ↔ Phase 3 independence — Phase 1 only depends on Phase 0, as the dependency graph promises.
