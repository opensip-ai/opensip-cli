...
[`packages/core/src/languages/adapter.ts`](../../../packages/core/src/languages/adapter.ts):
...
[`packages/core/src/languages/registry.ts`](../../../packages/core/src/languages/registry.ts) holds the in-memory list. Registration is by id; dispatch is by extension:
...
The CLI registers all six bundled adapters at module load time before any Tool runs. See [`packages/cli/src/index.ts:64-69`](../../../packages/cli/src/index.ts).
...
Each adapter ships in its own package. Each is a peer at Layer 3 of the [package graph](../10-mental-model/03-modular-monolith.md). The CLI imports all six directly; a Layer-5 dep on each.
...
`@opensip-tools/lang-typescript` re-exports `filterContent`, `clearFilterCache`, and `FilteredContent` from `@opensip-tools/fitness`. This is the documented exception to the "lang packs depend only on core" layer rule — see [`80-conventions/02-layer-policy.md`](../80-conventions/02-layer-policy.md) and the carve-out in [`.dependency-cruiser.cjs:171`](../../../.dependency-cruiser.cjs).
...
- **[`02-check-packs.md`](./02-check-packs.md)** — how check packs build on adapters: scope filters, parameterization, the marketplace shape.
- **[`../70-reference/01-package-catalog.md`](../70-reference/01-package-catalog.md)** — every adapter package, key exports, where to find them.
