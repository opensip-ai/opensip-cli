...
`@opensip-tools/lang-typescript` re-exports `filterContent`, `clearFilterCache`, and `FilteredContent` from `@opensip-tools/fitness`. This is the documented exception to the "lang packs depend only on core" layer rule — see [`90-conventions/02-layer-policy.md`](../90-conventions/02-layer-policy.md) and the named `lang-no-fitness-except-typescript` carve-out in [`.dependency-cruiser.cjs`](../../../.dependency-cruiser.cjs).
...
- **[`../80-reference/01-package-catalog.md`](../80-reference/01-package-catalog.md)** — every adapter package, key exports, where to find them.
