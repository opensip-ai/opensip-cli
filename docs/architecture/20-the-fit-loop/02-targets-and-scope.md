...
The shape lives in [`packages/fitness/engine/src/targets/types.ts`](../../../packages/fitness/engine/src/targets/types.ts):
...
[`packages/fitness/engine/src/framework/path-matcher.ts`](../../../packages/fitness/engine/src/framework/path-matcher.ts) is the per-check matcher object. It compiles include/exclude globs once and answers `match(filePath)` queries with a single pass through the compiled `Minimatch` instances.
...
- **[`../60-subsystems/01-language-adapters.md`](../60-subsystems/01-language-adapters.md)** — how a check's `contentFilter` setting dispatches through a per-language adapter.
- **[`../80-reference/02-configuration.md`](../80-reference/02-configuration.md)** — the full `targets:` schema in `opensip-tools.config.yml`.
