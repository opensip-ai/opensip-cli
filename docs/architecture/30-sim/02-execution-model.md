...
All four executors live under [`packages/simulation/engine/src/kinds/<kind>/executor.ts`](../../../packages/simulation/engine/src/kinds/). Each accepts a kind-specific config (validated at `define*Scenario` time) and a `ScenarioContext` (signal/abort, logger, persona context for load/chaos).
...
[`packages/simulation/engine/src/kinds/fix-evaluation/executor.ts`](../../../packages/simulation/engine/src/kinds/fix-evaluation/executor.ts)
...
2. For each signal: invoke the configured agent to generate a fix, score the output with each `predicate` ([`packages/simulation/engine/src/kinds/fix-evaluation/predicates/`](../../../packages/simulation/engine/src/kinds/fix-evaluation/predicates/)).
...
This is the union member that the renderer consumes (the `App.tsx` dispatcher in [`packages/cli/src/ui/`](../../../packages/cli/src/ui/) switches on `result.type`). It's also the shape `--json` serializes.
...
- **[`../80-internals/`](../80-internals/)** — how the CLI dispatches both fit and sim. Plugin loading, session writing, persistence layout.
