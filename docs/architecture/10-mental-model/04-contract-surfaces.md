...
The full per-field reference (when each field is present, what each value can be) is in [`80-reference/03-json-output-schema.md`](../80-reference/03-json-output-schema.md).
...
The gate's identity hash for diff matching is **not** SARIF-spec — it's an opensip-tools internal: `sha256(filePath + '\n' + ruleId + '\n' + message)`, deliberately excluding line numbers so unrelated line shifts don't register as added/resolved. See [`packages/fitness/engine/src/gate.ts:243`](../../../packages/fitness/engine/src/gate.ts) and [`60-subsystems/03-architecture-gate.md`](../60-subsystems/03-architecture-gate.md).
