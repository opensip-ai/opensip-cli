...
A Tool is a TypeScript object. Five fields, two methods. The whole interface lives at [`packages/core/src/tools/types.ts`](../../../packages/core/src/tools/types.ts):
...
- **`metadata.id`** is the registry key. `defaultToolRegistry.register(t)` writes `tools[t.metadata.id] = t`. Re-registering the same id is a no-op (last writer wins) — see [`packages/core/src/tools/registry.ts`](../../../packages/core/src/tools/registry.ts). This is how a third-party Tool can override a first-party one.
...
- **[`../70-surfaces/02-plugin-authoring.md`](../70-surfaces/02-plugin-authoring.md)** — full walkthrough of writing a Tool, a check pack, and a project-local check.
