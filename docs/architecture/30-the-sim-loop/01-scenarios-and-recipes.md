...
3. **npm-package.** Any package whose `package.json` declares `opensipTools.kind === 'sim-scenarios'`. The package's main entry exports `scenarios: RunnableScenario[]` and optionally `recipes: SimulationRecipe[]`.
