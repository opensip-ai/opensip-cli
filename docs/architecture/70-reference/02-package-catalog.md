...
2. **Add the dep-cruiser carve-out** if needed. The default layer rules forbid most cross-layer edges; if your package needs an exception, add it to [`.dependency-cruiser.cjs`](../../../.dependency-cruiser.cjs) and document it in [`../80-internals/05-layer-policy.md`](../80-internals/05-layer-policy.md).
