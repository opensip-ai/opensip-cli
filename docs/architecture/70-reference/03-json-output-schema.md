...
For SARIF (the gate's native shape), use `--gate-save` / `--gate-compare`. The SARIF shape is the SARIF 2.1.0 spec's, not opensip-tools' — see [`50-subsystems/03-architecture-gate.md`](../50-subsystems/03-architecture-gate.md).
