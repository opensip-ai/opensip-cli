...
First-party Tools (`fit`, `sim`, `graph`) are imported statically by the CLI. Third-party Tools are discovered by walking `node_modules` for any package whose `package.json` declares `opensipTools.kind === 'tool'`. See [`../10-mental-model/02-tool-plugin-model.md`](../10-mental-model/02-tool-plugin-model.md).
