...
opensip-tools is a platform with two tools shipped today, designed for a third you haven't installed yet. The CLI ([`packages/cli/src/index.ts`](../../../packages/cli/src/index.ts)) is a generic dispatcher: it walks `defaultToolRegistry`, calls `Tool.register(cli)` on each entry, and lets each tool wire its own Commander commands. Adding a new tool is a plugin operation — install a package whose `package.json` declares `opensipTools.kind === 'tool'`, and the CLI picks it up.

This is not a hypothetical: it's why `fit` and `sim` ship in separate packages, depend on the same kernel ([`@opensip-tools/core`](../../../packages/core/)), and have completely separate command surfaces. If `fit` ever wanted to know what `sim` was doing, it'd have to import it — which the layer policy forbids. They communicate through the CLI's render layer, not directly.
...
The kernel ships zero adapters. The CLI binds the six bundled adapters at startup ([`packages/cli/src/index.ts:64-69`](../../../packages/cli/src/index.ts)). A polyglot project gets every relevant pack; a single-language project still loads them all (cheap) and only invokes the relevant ones (per-file dispatch).
