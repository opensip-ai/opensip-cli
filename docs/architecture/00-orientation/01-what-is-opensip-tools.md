...
You're joining the project, or you're about to depend on it. Before any package layout, any CLI flag, any plugin contract — start here. Read this doc, then [`02-vocabulary.md`](./02-vocabulary.md), then [`03-system-context.md`](./03-system-context.md). After those three, the rest of the doc set is just depth on parts you already understand at a sketch level.
...
The whole loop is described in detail in [`../10-mental-model/01-fitness-loop.md`](../10-mental-model/01-fitness-loop.md), which is the spine of this doc set.
...
opensip-tools is a platform with three tools shipped today (`fit`, `sim`, `graph`), designed for a fourth you haven't installed yet. The CLI ([`packages/cli/src/index.ts`](../../../packages/cli/src/index.ts)) is a generic dispatcher: it walks `defaultToolRegistry`, calls `Tool.register(cli)` on each entry, and lets each tool wire its own Commander commands. Adding a new tool is a plugin operation — install a package whose `package.json` declares `opensipTools.kind === 'tool'`, and the CLI picks it up.

This is not a hypothetical: it's why `fit`, `sim`, and `graph` ship in separate packages, depend on the same kernel ([`@opensip-tools/core`](../../../packages/core/)), and have completely separate command surfaces. If `fit` ever wanted to know what `sim` was doing, it'd have to import it — which the layer policy forbids. They communicate through the CLI's render layer, not directly.

### Layered, not modular

The 27 packages are organized as a strict dependency layer cake: `core` at the bottom, then `datastore` + `contracts` above it, then `fitness`/`simulation`/`graph`/`dashboard`/`cli-ui`/`lang-*`/`graph-*` as peers, then `checks-*` packs at Layer 4, then `cli` at the top. The layer policy is enforced by [dependency-cruiser](../../../.dependency-cruiser.cjs) at lint time — the build fails if a `core` module imports from `fitness`, or if `lang-typescript` imports from `cli`.
...
See [`../10-mental-model/03-modular-monolith.md`](../10-mental-model/03-modular-monolith.md) for the full layer map.

### Polyglot via adapters

opensip-tools runs on TypeScript, but the checks it runs apply to TypeScript, Rust, Python, Java, Go, and C/C++ code. The trick is the `LanguageAdapter` interface ([`packages/core/src/languages/adapter.ts`](../../../packages/core/src/languages/adapter.ts)): each language pack contributes one adapter that knows how to strip comments and string literals from that language's source. Checks operate on the *filtered* content, so a regex like `/console\.log/` doesn't match the literal string `"console.log"` inside a JS comment.

The kernel ships zero adapters. The CLI binds the six bundled adapters at startup ([`packages/cli/src/index.ts:68-73`](../../../packages/cli/src/index.ts)). A polyglot project gets every relevant pack; a single-language project still loads them all (cheap) and only invokes the relevant ones (per-file dispatch).
...
- **[`02-vocabulary.md`](./02-vocabulary.md)** — the words used everywhere in this codebase. *Tool, recipe, check, scenario, signaler, target, language adapter, plugin, session.* Reading this is the highest-leverage 15 minutes you'll spend on onboarding — most other docs assume you know the vocabulary.
- **[`03-system-context.md`](./03-system-context.md)** — the box diagram. Where the binary lives, what it touches on disk, the user-level vs. project-level split.

After orientation, you want the mental-model section ([`../10-mental-model/`](../10-mental-model/)). Start with [`01-fitness-loop.md`](../10-mental-model/01-fitness-loop.md) — it's the spine of the doc set and threads a single check end-to-end through the whole system.
