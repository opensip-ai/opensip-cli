...
`docs/web/` in this repo is generated, deterministic, and committed. The website at opensip.ai fetches those files from GitHub at runtime — no build coupling between the two repos. This document is the contract.
...
The single entry point is `docs/web/manifest.json`. Fetch it from:

```
https://raw.githubusercontent.com/opensip-ai/opensip-tools/main/docs/web/manifest.json
```
...
type DocPage = {
  file: string;          // 'docs/web/00-orientation/00-quick-start.md'
  path: string;          // '/docs/opensip-tools/00-orientation/00-quick-start/'
  section: string;       // '00-orientation' (empty string for root README)
  title: string;
  audience?: string[];
  purpose?: string;
};
...
type NavSection = {
  section: string;       // '00-orientation'
  title: string;         // '00 — Orientation'
...
The contract is: every time `docs/web/manifest.json` in opensip-tools updates, the website picks up the changes within the 5-minute TTL window. No deployment of opensip.ai is needed for new docs.

In opensip-tools:
1. Edit `docs/architecture/*.md`
2. `pnpm docs:build` regenerates `docs/web/` (including manifest)
3. Commit and push

The CI sync check ensures `docs/web/` never drifts from `docs/architecture/`.
