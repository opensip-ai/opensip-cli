...
> opensip-tools fit
  Fitness Checks
  Recipe: default   Checks: 158   Project: ~/work/my-app
...
The third loop. *"What is reachable from where?"* Builds the project's static call graph in five stages and runs five built-in rules over it: `orphan-subtree`, `duplicated-function-body`, `no-side-effect-path`, `test-only-reachable`, `always-throws-branch`. Five language adapters ship in v2.0 (TypeScript, Python, Rust, Go, Java). Has its own baseline-gate flow.
