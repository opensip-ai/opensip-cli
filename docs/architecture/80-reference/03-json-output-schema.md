...
For SARIF (the gate's native shape), use `--gate-save` / `--gate-compare`. The SARIF shape is the SARIF 2.1.0 spec's, not opensip-tools' — see [`60-subsystems/03-architecture-gate.md`](../60-subsystems/03-architecture-gate.md).

---

## What's next

- **[`02-configuration.md`](./02-configuration.md)** — `opensip-tools.config.yml` schema (the *input* shape).
- **[`../10-mental-model/04-contract-surfaces.md`](../10-mental-model/04-contract-surfaces.md)** — every contract surface, with stability tiers.
