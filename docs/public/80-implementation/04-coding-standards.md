...
The workspace root [`tsconfig.json`](../../../tsconfig.json) sets `target: ES2022`, `module: Node16`, `moduleResolution: Node16`, and `strict: true`. Each package has its own `tsconfig.json` that extends those settings.
...
Flat config at [`.config/eslint.config.mjs`](../../../.config/eslint.config.mjs). The base layers:
...
- `state.lock.acquire.complete` / `state.lock.acquire.timeout` — datastore or artifact write lock outcome ([ADR-0075](../../decisions/ADR-0075-state-locking-and-baseline-identity-versioning.md)).
...
Type-only imports use `import type` so they're erased at compile time. The main dep-cruiser pass ignores type-only imports (`tsPreCompilationDeps: false`) because they carry no runtime edge — but this does **not** mean you may `import type` from a higher layer. A second, type-aware pass (`.config/dependency-cruiser.types.cjs`, `tsPreCompilationDeps: true`) re-runs the full layer ruleset over the type-inclusive graph, so a type-only layer inversion or cycle is still rejected. Both passes run under `pnpm lint`. See [`05-layer-policy.md`](./05-layer-policy.md#how-to-add-a-new-exception) and [`../10-concepts/03-modular-monolith.md`](../10-concepts/03-modular-monolith.md#type-only-edges-are-caught-by-the-type-aware-pass).
...
- **[`00-repository-layout.md`](./00-repository-layout.md)** — ownership and maintenance rules for root files and directories.
- **[`05-layer-policy.md`](./05-layer-policy.md)** — the dep-cruiser config, rule by rule, with rationale.
- **[`06-doc-conventions.md`](./06-doc-conventions.md)** — voice, frontmatter, and verification trails for documentation.
- **[`../70-reference/02-package-catalog.md`](../70-reference/02-package-catalog.md)** — the workspace package list these standards apply to.
