...
hooks ([ADR-0076](../../decisions/ADR-0076-tool-authoring-template-and-helper-boundary.md)).
...
If your tool also wants to ship checks (the way `@opensip-cli/checks-typescript` does for `fit`), that's a separate, lighter contract — a check pack declaring the `fit-pack` marker plus target-domain epoch. See [`50-extend/01-plugin-authoring.md`](../50-extend/01-plugin-authoring.md).
...
- **[`../50-extend/07-command-taxonomy.md`](../50-extend/07-command-taxonomy.md)** — Tier-1/2/3 command grammar for Tool authors.
