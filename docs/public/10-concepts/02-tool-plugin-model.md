...
Since the **3.0.0 GA cutover** ([ADR-0027](../../decisions/ADR-0027-ga-parity-cutover.md)) this goes one step further: a bundled tool (`fit`/`sim`/`graph`) and an installed or project-local one load through the *same* path. The only thing distinguishing them is their **source of installation, never their lifecycle**.
...
That's the whole tool. `npm install @yourorg/audit-sec` (or `opensip-tools plugin add @yourorg/audit-sec`), and `opensip-tools audit-sec` works. For the full walkthrough — installation modes, per-command options, kernel-registry reuse — see [Full Tool plugins](../50-extend/06-full-tool-plugins.md).
