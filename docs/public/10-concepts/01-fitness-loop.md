...
4. Hand argv to Commander, which dispatches to `fitnessTool`'s `fit` action handler ([`packages/fitness/engine/src/tool.ts`](../../../packages/fitness/engine/src/tool.ts) — the `program.command(FIT.name).action(...)` block; the `dashboard`, `fit-list`, and `fit-recipes` action handlers live alongside it).
...
Source: [`packages/core/src/plugins/discover.ts`](../../../packages/core/src/plugins/discover.ts), [`packages/fitness/engine/src/plugins/`](../../../packages/fitness/engine/src/plugins/), [`packages/cli/src/bootstrap/register-language-adapters.ts`](../../../packages/cli/src/bootstrap/register-language-adapters.ts) (language-adapter registration).
