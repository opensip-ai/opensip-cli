...
supersedes: []
superseded_by: null
related: [ADR-0081, ADR-0061, ADR-0011]
...
declaration is **honest labeling, not a sandbox** — enforcement of `requires`
is deferred to spec 03 (Gate A), consistent with ADR-0081. An adapter is listed
as **recommended** only after a security-review bar: first-party adapters take a
fast-path (opensip.ai-authored, reviewed in-repo); third-party adapters are gated
on 03's Gate C consumption-side verification. Independently, **no raw scanner
...
- **Enforce `requires` now as a capability sandbox** — rejected; no capability
  sandbox exists yet (ADR-0081 made the same call for capability packs). Implying
  enforcement would mislead. `requires` is a declaration in v1; enforcement lands
  with 03.
- **Auto-recommend any installable adapter** — rejected; "recommended" is a trust
  signal. A first-party fast-path plus a third-party security bar (03 Gate C) keeps
...
and operators an honest, machine-readable surface today, and becomes enforceable
the moment 03's capability model ships — with no contract change. The MVP's three
...
- **`requires` stays declaration-only in v1.** Enforcement is inherited from spec
  03 (Gate A); until then the field is a reviewed, hash-covered manifest fact (the
  ADR-0081 normalization).
- **Recommended-listing bar:** first-party adapters fast-path through in-repo
  security review; third-party adapters are gated on 03 Gate C
  (consumption-side verification, ADR-0068). Installation of a non-recommended
  adapter remains possible (deny-by-default trust opt-in, ADR-0054/0061) — listing
  is the trust signal, not the gate to install.
...
|-----------|-----------|-------------|
| Recommended-listing security bar (an adapter is listed as recommended only after review) | **No check warranted** | **Not mechanizable** — a security review is a human judgment; no fitness check / depcruise rule can assert "this code was reviewed." The bar is a process gate (first-party fast-path; third-party 03 Gate C), recorded here as policy. |
...
| `requires` is declaration-only in v1 (no capability sandbox) | **No check warranted** | Policy boundary inherited from ADR-0081/ADR-0061; enforcement deferred to spec 03 Gate A. A check would imply an enforcement that does not yet exist. |
...
(the launch posture and trust tiers; public ecosystem stays gated), and
