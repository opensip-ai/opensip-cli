...
- [ADR-0169](ADR-0169-cache-first-runtime-evidence-continuity.md) — Cache-first local evidence: no-init analysis writes to the user cache; `opensip init` transactionally adopts that exact evidence into project `.runtime`; project-scoped MCP serves the same store before Init; exact `report --run` selection uses run-addressed artifacts; shared/exclusive runtime leases and crash-recoverable uninstall protect the journey.
