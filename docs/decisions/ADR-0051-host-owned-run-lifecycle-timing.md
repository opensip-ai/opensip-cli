...
tags: [sessions, timing, persistence, dashboard, hygiene, architecture, contracts]
...
they RETURN a `ToolRunCompletion` (`{ result?, envelope?, session?, dashboard? }`)
...
records host-side overhead on a sibling `StoredSessionHostMetrics` record, and
persists any returned per-run `ToolDashboardContribution`. The public
`ToolCliContext.runSession` carries only a read-only `timing` for a display
clock; the transitional `runSession.record(...)` writer is removed.
...
- `ToolRunCompletion`, `ToolSessionContribution`, `ToolDashboardContribution`,
  `RunTimer`/`RunLifecycle`, `StoredSessionHostMetrics` are the new contract
  types; `ToolCliContext.runSession` is `{ timing }` only.
...
- `buildToolCliContext` is a thin assembler over focused bootstrap planes
  (run / live / output / egress / dashboard / baseline / state / host-planes).
