...
export function checkScenario(result, expect = {}) {
  ...
  if (expect.exitCodeOneOf !== undefined && !expect.exitCodeOneOf.includes(result.exitCode)) {
    failures.push(
      `exitCode: expected one of [${expect.exitCodeOneOf.join(', ')}], got ${result.exitCode}`,
    );
  }
  ...
  if (expect.json !== undefined) {
    ...
    try {
      ...
      failures.push(
        `stdout is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
      );
      return failures;
    }
...
export function expectEnvelope(opts = {}) {
  return (outcome) => {
    ...
    if (parsed.schemaVersion !== 2) {
      failures.push(
        `envelope.schemaVersion: expected 2, got ${JSON.stringify(parsed.schemaVersion)}`,
      );
    }
    if (opts.tool !== undefined && parsed.tool !== opts.tool) {
      failures.push(
        `envelope.tool: expected ${JSON.stringify(opts.tool)}, got ${JSON.stringify(parsed.tool)}`,
      );
    }
