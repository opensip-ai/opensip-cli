/**
 * @fileoverview Benchmark-facing re-export of the shared parent-signal
 * coordinator + per-child terminator.
 *
 * The implementation moved to `scripts/lib/measured-process.mjs` so both the
 * benchmarks and platform acceptance share ONE bounded measured-process
 * substrate without acceptance depending on benchmark ownership. This shim keeps
 * every existing `scripts/perf/child-process-lifecycle.mjs` import stable.
 */

export {
  createChildProcessTerminator,
  createParentSignalCoordinator,
  parentSignalCoordinator,
  reserveParentSignalCleanup,
} from '../lib/measured-process.mjs';
