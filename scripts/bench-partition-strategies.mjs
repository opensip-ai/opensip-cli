...
// toggled ONLY by rewriting the corpus's opensip-cli.config.yml, and every
...
//   C  (cold):  delete <corpus>/opensip-cli/.runtime/, then
...
// name needs to be known). The corpus's `opensip-cli.config.yml` is
...
function writeCorpusConfig(strategy) {
  ...
  writeFileSync(
    join(corpusDir, 'opensip-cli.config.yml'),
...
function benchStrategy(strategy) {
  ...
  rmSync(join(corpusDir, 'opensip-cli', '.runtime'), {
    recursive: true,
    force: true,
  });
  const colds = [];
