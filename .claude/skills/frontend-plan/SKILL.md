...
Plan to those two surfaces only. Plans live in the local-only (gitignored) plans
scratch as a directory with a top-level `plan.md` and separate `phase-N-name.md`
files.
...
Read 1-2 existing ready plans for the structural format,
and `AGENTS.md` (layer DAG, the documented
