...
/**
 * Current logical schema stamp for the bundled post-squash journal. Five entries:
 * the squash (0000) + the `stable_id` drop (0001) + `run_outcome` (0002) +
 * baseline identity meta (0003) + suite grouping (0004). Bump this in lockstep
 * with the bundled journal entry count (or fold into the offset on the next squash).
 */
export const LOGICAL_SCHEMA_VERSION = SCHEMA_VERSION_OFFSET + 5;
