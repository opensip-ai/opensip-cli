...
describe('validateBookkeeping', () => {
  ...
  it('flags an allowlist entry that no longer ships', () => {
    const problems = validateBookkeeping(coverageConfig({ checks: [check({ slug: 'a' })], allowlist: ['gone'] }))
  ...
  it('flags a command-mode check that is not exempted', () => {
    const problems = validateBookkeeping(coverageConfig({ checks: [check({ slug: 'cmd', analysisMode: 'command' })] }))
