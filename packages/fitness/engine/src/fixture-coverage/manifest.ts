...
function allowlistProblems(
  ...
  shipped: ReadonlyMap<string, CheckConfig>,
): string[] {
...
function exemptionProblems(
  ...
  for (const check of config.checks) {
    ...
    if (
      cfg.disabled !== true &&
      cfg.analysisMode === 'command' &&
      !(cfg.slug in config.commandExemptions)
    ) {
      problems.push(
...
function unfixturableProblems(
  ...
  for (const [slug, reason] of Object.entries(config.knownUnfixturable ?? {})) {
    ...
    if (!cfg) {
      ...
      problems.push(
        `'${slug}' is analysisMode:'command' — put it in commandExemptions, not knownUnfixturable`,
      );
    }
    if (reason.length === 0) {
