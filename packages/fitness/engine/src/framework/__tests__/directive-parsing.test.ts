...
describe('parseFileIgnoreDirective — comment-prefix support', () => {
  ...
  it('recognises `<!--` (Markdown + HTML)', () => {
    const content =
      '<!-- @fitness-ignore-file file-length-limit -- doc-set catalogue grows by design -->\n# Heading';
    expect(parseFileIgnoreDirective(content, 'file-length-limit')).toBe(true);
  });

  it('recognises `#` (shell / YAML / Python)', () => {
    const content =
      '# @fitness-ignore-file file-length-limit -- config grows by design\nkey: value';
    expect(parseFileIgnoreDirective(content, 'file-length-limit')).toBe(true);
  });

  it('does not recognise an unsupported prefix', () => {
    const content =
      '; @fitness-ignore-file file-length-limit -- ini-style comments not supported\n[section]';
