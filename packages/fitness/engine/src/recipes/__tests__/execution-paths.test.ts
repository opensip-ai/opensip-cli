...
describe('check-result-processor — error result fields', () => {
  ...
  it('preserves non-timeout error details', async () => {
    ...
    checkRegistry.register(defineCheck({
      ...
      analyzeAll: async () => { throw new Error('explicit failure') },
    }))
    writeFixture('a.ts', '')
...
describe('catalog sync', () => {
  it('invokes onCatalogSync with all registered checks before execution', async () => {
    ...
    const callbacks: FitnessRecipeServiceCallbacks = {
      onCatalogSync: (entries) => { received = entries.map((e) => ({ id: e.id, slug: e.slug })) },
    }
    const checkRegistry = new CheckRegistry()
    checkRegistry.register(defineCheck({
      id: uid(), slug: 'c1', description: 'd', tags: ['quality'], analyze: () => [],
    }))
    checkRegistry.register(defineCheck({
      id: uid(), slug: 'c2', description: 'd', tags: ['quality'], analyze: () => [],
    }))
    writeFixture('a.ts', '')
