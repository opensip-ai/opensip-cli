// @fitness-ignore-file shipped-checks-must-be-generic -- opensip-internal dogfood check (ADR-0139): path-gated to packages/cli and packages/external-tool-adapter; inert for adopters.
/**
 * @fileoverview External-adapter progress bridge privacy.
 *
 * ADR-0139/ADR-0090/ADR-0054 keep installed adapter runtime code isolated in a worker.
 * The host-owned live view needs a narrow progress bridge, but that bridge is
 * private to the CLI worker shim and the external-tool-adapter substrate. This
 * check prevents other packages from importing the private progress symbol or
 * helper functions and turning them into a de facto public ToolCliContext seam.
 */
...
const NON_SOURCE = /(?:\.test\.tsx?$|\/__tests__\/|external-adapter-progress-private-bridge\.ts$)/;
