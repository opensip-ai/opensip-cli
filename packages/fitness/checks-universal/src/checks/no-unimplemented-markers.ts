 ...
 */
function lineMatcher(re: RegExp, messageTest?: (arg: string) => boolean): (line: string) => boolean {
  return (line) => {
...
const CPP_PATTERNS: readonly MarkerPattern[] = [
  ...
  { marker: 'assert(... && "not implemented")', test: lineMatcher(/\bassert\s*\((.*)\)/, isUnimplementedArg) },
]
