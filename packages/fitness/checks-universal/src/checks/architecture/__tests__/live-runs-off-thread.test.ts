...
describe('analyzeLiveRunsOffThread', () => {
  it('flags a live runner that calls the bare in-process transport', () => {
    const v = analyzeLiveRunsOffThread('const run = createInProcessTransport().run(fn);', 'cli/fit-runner.tsx')
  ...
  it('flags a worker entry that persists', () => {
    const v = analyzeLiveRunsOffThread('persistSession({ cwd }, signals, ds, ms);', 'cli/graph-worker.ts')
    expect(v).toHaveLength(1)
  ...
  it('passes a runner that uses the off-thread selector', () => {
    expect(
      analyzeLiveRunsOffThread('const run = runOffThreadOrInProcess({ descriptor, inProcess });', 'cli/sim-runner.tsx'),
    ).toEqual([])
  })

  it('passes a worker that reads the scope datastore but does not persist', () => {
    expect(
      analyzeLiveRunsOffThread('const ds = cli.scope.datastore(); await runEngine(spec, ds);', 'cli/graph-worker.ts'),
    ).toEqual([])
  })
  ...
  it('only the runner rule applies to runners (a persist call in a runner is not flagged by the worker rule)', () => {
    ...
    const v = analyzeLiveRunsOffThread('persistSession(x); createInProcessTransport();', 'cli/fit-runner.tsx')
    expect(v).toHaveLength(1)
  })

  it('ignores files that are neither runners nor workers', () => {
    expect(analyzeLiveRunsOffThread('createInProcessTransport(); persistSession(x);', 'cli/fit-modes.ts')).toEqual([])
  })

  it('skips test files even when named like a runner/worker', () => {
    expect(analyzeLiveRunsOffThread('createInProcessTransport();', 'cli/__tests__/fit-runner.test.tsx')).toEqual([])
