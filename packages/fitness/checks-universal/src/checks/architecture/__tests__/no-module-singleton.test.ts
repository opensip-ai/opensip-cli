...
import {
  analyzeAllNoModuleSingleton,
  analyzeNoModuleSingleton,
} from '../no-module-singleton.js'
...
describe('analyzeNoModuleSingleton (pure detector)', () => {
  it('flags a re-added registry singleton', () => {
    const v = analyzeNoModuleSingleton(
  ...
  it('flags a module-level EventEmitter / Map singleton', () => {
    expect(
      analyzeNoModuleSingleton('export const bus = new EventEmitter()\n', SRC),
    ).toHaveLength(1)
    expect(
      analyzeNoModuleSingleton('export const cache = new Map()\n', SRC),
    ).toHaveLength(1)
  })
  ...
  it('does not match a non-mutable constructor', () => {
    expect(
      analyzeNoModuleSingleton('export const url = new URL("x")\n', SRC),
    ).toEqual([])
    expect(
      analyzeNoModuleSingleton('export const re = new RegExp("x")\n', SRC),
    ).toEqual([])
  })
  ...
  it('does not match a factory return', () => {
    const factory =
      'export function createCheckRegistry() { return new CheckRegistry() }\n'
