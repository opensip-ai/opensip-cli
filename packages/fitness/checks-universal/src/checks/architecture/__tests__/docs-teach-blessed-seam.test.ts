...
describe('analyzeBlessedSeam', () => {
  ...
  it('flags a raw process.stdout.write in a doc example', () => {
    expect(analyzeBlessedSeam('        process.stdout.write(JSON.stringify(result));').length).toBeGreaterThanOrEqual(1)
  })

  it('does NOT flag the blessed seam (commandSpecs / defineCommand / emit seams)', () => {
    expect(analyzeBlessedSeam("commandSpecs: [defineCommand({ name: 'x', commonFlags: ['json'] })]")).toHaveLength(0)
