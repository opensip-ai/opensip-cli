...
describe('analyzeRawDbAccess', () => {
  it('flags `.db.select(` outside the persistence boundary', () => {
    const violations = analyzeRawDbAccess('const r = this.datastore.db.select().from(t).get();', OUTSIDE)
  ...
  it('flags every Drizzle query/builder method', () => {
    for (const m of ['select', 'insert', 'update', 'delete', 'transaction', 'run', 'get', 'all', 'values', 'with']) {
  ...
  it('does NOT flag `.db.select(` INSIDE a src/persistence/ layer', () => {
    ...
    expect(analyzeRawDbAccess('this.datastore.db.select().from(t).get();', persistence)).toHaveLength(0)
  })

  it('does NOT flag `.db` use in session-store or datastore packages', () => {
    expect(analyzeRawDbAccess('this.datastore.db.delete(sessions).run();', 'packages/session-store/src/session-repo.ts')).toHaveLength(0)
    expect(analyzeRawDbAccess('migrate(datastore.db, { migrationsFolder });', 'packages/datastore/src/factory.ts')).toHaveLength(0)
  })

  it('does NOT flag test files (fixtures legitimately reach the handle)', () => {
    expect(analyzeRawDbAccess('store.db.select().from(t)', 'packages/foo/src/x.test.ts')).toHaveLength(0)
    expect(analyzeRawDbAccess('store.db.select().from(t)', 'packages/foo/src/__tests__/x.ts')).toHaveLength(0)
  })
  ...
  it('does NOT flag an unrelated `.db` field that is not a query handle', () => {
    ...
    expect(analyzeRawDbAccess('const name = options.db.name ?? "default";', OUTSIDE)).toHaveLength(0)
