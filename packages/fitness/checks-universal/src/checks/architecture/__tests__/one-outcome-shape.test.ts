...
describe('analyzeOneOutcomeShape', () => {
  ...
  it('flags a direct stdout JSON write outside the renderer', () => {
    expect(
      analyzeOneOutcomeShape('process.stdout.write(JSON.stringify(outcome))', false),
    ).toHaveLength(1);
    expect(
      analyzeOneOutcomeShape('process.stdout.write(formatSignalJson(env))', false),
    ).toHaveLength(1);
  });

  it('allows the stdout JSON write inside the renderer (render-outcome.ts)', () => {
    expect(
      analyzeOneOutcomeShape('process.stdout.write(JSON.stringify(outcome))', true),
    ).toHaveLength(0);
  });
