...
const FORBIDDEN: readonly { readonly re: RegExp; readonly what: string }[] = [
  { re: /\.option\(\s*['"]--json['"]/, what: "a hand-rolled `.option('--json')` — the host provides `--json` via commonFlags" },
  { re: /\bprocess\.stdout\.write\s*\(/, what: 'a raw `process.stdout.write(...)` — return your result; the host renders + serializes it' },
  { re: /\bstdout\.write\s*\([^)]*JSON\.stringify/, what: 'a JSON.stringify-to-stdout write — the host wraps `--json` in a CommandOutcome' },
]

/** Pure analysis over one markdown doc's content. Exported for unit tests. */
export function analyzeBlessedSeam(content: string): { readonly line: number; readonly what: string }[] {
...
export const docsTeachBlessedSeam = defineCheck({
  ...
  description: 'Extension docs must teach the blessed CommandSpec seam, not hand-rolled --json / stdout (§4.8)',
