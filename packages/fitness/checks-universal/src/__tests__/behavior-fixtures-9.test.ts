...
describe('typescript-directive-hygiene separators', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'src/colon.ts',
      ['// @ts-expect-error: legacy API has incorrect upstream types', 'const x = legacy()'].join(
        '\n',
      ),
    );
    // `- ` separator with a real reason -> accepted, no violation.
    ...
    writeFixture(cwd, 'src/empty-reason.ts', ['// @ts-ignore --', 'const z = legacy()'].join('\n'));
    // `:` separator with empty reason -> missing justification.
    writeFixture(cwd, 'src/empty-colon.ts', ['// @ts-ignore:', 'const w = legacy()'].join('\n'));
  });
...
describe('image-optimization', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'src/expo-missing.tsx',
      ["import { Image } from 'expo-image'", 'export const C = () => <Image source={src} />'].join(
        '\n',
      ),
    );
...
describe('isInPublicApiSurface resolution edge cases', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'pkg-src/src/index.ts', ["export { foo } from './lib'"].join('\n'));
