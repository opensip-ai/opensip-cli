...
describe('public-api-jsdoc branches', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/internal/helper.ts', [
      'export function _private() { return 3; }',
    ].join('\n'))
    // Test file - skipped
    ...
    // .d.ts file - skipped
    writeFixture(cwd, 'src/types.d.ts', [
      'export function dts() { return 5; }',
    ].join('\n'))
  })
...
describe('no-process-exit-in-finally', () => {
  ...
  beforeAll(() => {
    ...
    // No finally
    writeFixture(cwd, 'src/no-finally.ts', [
      'try { doSomething(); }',
...
describe('docker-best-practices more', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'Dockerfile.apt', [
      'FROM ubuntu:22.04',
...
describe('empty-package-detection', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'packages/full/src/index.ts', [
...
describe('dependency-security-audit parseOutput', () => {
  it('parses npm audit JSON with vulnerabilities', async () => {
    ...
    const cmd = (mod.dependencySecurityAudit as { config: { commandConfig?: unknown } }).config
