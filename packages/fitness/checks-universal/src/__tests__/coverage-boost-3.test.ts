...
describe('sentry-source-maps', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'rollup.config.ts', [
...
describe('env-secret-exposure', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/log-env.ts', [
    ...
    writeFixture(cwd, 'src/json-env.ts', [
      'export function dump() {',
      '  return JSON.stringify(process.env);',
      '}',
    ].join('\n'))
    writeFixture(cwd, 'src/spread-env.ts', [
      'export const settings = { ...process.env, custom: 1 };',
    ].join('\n'))
...
describe('dangerous-config-defaults TLS variant', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/tls.ts', [
      'process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0";',
    ].join('\n'))
    writeFixture(cwd, 'src/quoted.ts', [
      'export const env = {',
...
describe('no-stub-tests function form', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/__tests__/fn.test.ts', [
      'import { it } from "vitest";',
      'it("plain function form", function() {});',
    ].join('\n'))
  })
...
describe('public-api-jsdoc', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/no-doc.ts', [
      'export function thing() { return 1; }',
      'export class Box { open() {} }',
    ].join('\n'))
    writeFixture(cwd, 'src/with-doc.ts', [
      '/**',
      ' * Returns one.',
      ' */',
      'export function thing() { return 1; }',
...
describe('docker-best-practices additional', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'Dockerfile.no-healthcheck', [
      'FROM node:20',
      'COPY . .',
      'CMD ["node", "src/app.js"]',
    ].join('\n'))
  ...
  it('runs without throwing on docker fixtures', async () => {
    const result = await findCheck('docker-best-practices').run(cwd, {
      targetFiles: [
        join(cwd, 'Dockerfile.latest'),
        join(cwd, 'Dockerfile.no-healthcheck'),
      ],
    })
    expect(result).toBeDefined()
  })
...
describe('heavy-import-detection', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/lodash-namespace.ts', [
      'import * as _ from "lodash";',
      'export const x = _.pick({ a: 1 }, ["a"]);',
    ].join('\n'))
    writeFixture(cwd, 'src/moment.ts', [
      'import moment from "moment";',
      'export const x = moment(new Date()).format();',
    ].join('\n'))
    writeFixture(cwd, 'src/aws-sdk.ts', [
...
describe('stale-build-artifacts variants', () => {
  ...
  it('runs without throwing', async () => {
    const result = await findCheck('stale-build-artifacts').run(cwd, {
      targetFiles: [
        join(cwd, 'packages/a/dist/index.js'),
        join(cwd, 'packages/a/package.json'),
      ],
    })
    expect(result).toBeDefined()
...
describe('eslint-justifications', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/generic-reason.ts', [
...
describe('test-convention-consistency', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/__tests__/foo-spec.ts', [
      'import { it } from "vitest";',
      'it("works", () => undefined);',
    ].join('\n'))
    writeFixture(cwd, 'src/__tests__/bar.test.ts', [
      'import { it } from "vitest";',
      'it("works", () => undefined);',
    ].join('\n'))
  })
  ...
  it('runs without throwing on mixed naming forms', async () => {
    const result = await findCheck('test-convention-consistency').run(cwd, {
      targetFiles: [
        join(cwd, 'src/__tests__/foo-spec.ts'),
        join(cwd, 'src/__tests__/bar.test.ts'),
      ],
    })
    expect(result).toBeDefined()
  })
