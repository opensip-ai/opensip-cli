...
describe('sentry-environment-set / sentry-release-set branches', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/init-bare.ts', [
    ...
    writeFixture(cwd, 'src/init-env.ts', [
      'import * as Sentry from "@sentry/node";',
      'Sentry.init({',
      '  dsn: "https://example",',
      '  environment: "prod",',
    ...
    // Has release present
    writeFixture(cwd, 'src/init-rel.ts', [
      'import * as Sentry from "@sentry/node";',
      'Sentry.init({',
      '  dsn: "https://example",',
      '  release: "1.0",',
    ...
    writeFixture(cwd, 'src/no-init.ts', [
      'import * as Sentry from "@sentry/node";',
    ...
    writeFixture(cwd, 'src/unclosed.ts', [
      'import * as Sentry from "@sentry/node";',
      'Sentry.init(',
      '  // unclosed',
    ].join('\n'))
...
describe('sentry-error-boundary additional branches', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/no-jsx.tsx', [
...
describe('sentry-sample-rate branches', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/no-rate.ts', [
      'import * as Sentry from "@sentry/node";',
      'Sentry.init({ dsn: "https://example" });',
    ].join('\n'))
    // Reasonable rate (0.1)
    writeFixture(cwd, 'src/good-rate.ts', [
      'import * as Sentry from "@sentry/node";',
      'Sentry.init({',
      '  dsn: "https://example",',
...
describe('sentry-pii-scrubbing branches', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/pii-true.ts', [
      'import * as Sentry from "@sentry/node";',
      'Sentry.init({',
      '  dsn: "https://example",',
      '  sendDefaultPii: true,',
    ...
    writeFixture(cwd, 'src/pii-with-bs.ts', [
      'import * as Sentry from "@sentry/node";',
      'Sentry.init({',
      '  dsn: "https://example",',
      '  sendDefaultPii: true,',
    ...
    // No init (should skip)
    writeFixture(cwd, 'src/no-init-pii.ts', [
      'export const x = 1;',
    ].join('\n'))
  })
...
describe('sentry-helpers extractSentryInitBlock', () => {
  it('handles unclosed Sentry.init block as fallback', async () => {
    const { extractSentryInitBlock, hasSentryInit, hasSentryUsage } = await import(
      '../checks/resilience/sentry/sentry-helpers.js'
    )
    ...
    const unclosed = extractSentryInitBlock([
      'Sentry.init(',
      '  // never closed',
    ].join('\n'))
    ...
    const closed = extractSentryInitBlock([
      'Sentry.init({',
      '  dsn: "x",',
      '});',
    ].join('\n'))
...
describe('config-validation-helpers', () => {
  it('isDigit handles all cases', async () => {
    const { isDigit } = await import('../checks/resilience/config-validation-helpers.js')
  ...
  it('isAlphanumericChar handles all cases', async () => {
    const { isAlphanumericChar } = await import('../checks/resilience/config-validation-helpers.js')
  ...
  it('skipWhitespace advances past spaces and tabs', async () => {
    const { skipWhitespace } = await import('../checks/resilience/config-validation-helpers.js')
  ...
  it('parseDigits handles digit and non-digit starts', async () => {
    const { parseDigits } = await import('../checks/resilience/config-validation-helpers.js')
...
describe('dependency-security-audit parseOutput', () => {
  it('returns empty for clean exit code 0 and empty stdout', async () => {
    const { dependencySecurityAudit } = await import(
      '../checks/quality/dependency-security-audit.js'
    )
    const cmd = (dependencySecurityAudit as unknown as { config: { execute: unknown } })
    // The check has command mode — get the original config
    expect(cmd).toBeDefined()
  })

  it('parses npm audit JSON with vulnerabilities', async () => {
    const mod = await import('../checks/quality/dependency-security-audit.js')
    ...
    expect(mod.dependencySecurityAudit).toBeDefined()
  })
})
...
describe('security-scan-suite parseOutput drives via JSON', () => {
  ...
  it('check is registered', () => {
    const c = findCheck('security-scan-suite')
    expect(c.config.slug).toBe('security-scan-suite')
  })
})
...
describe('hasura-production-config missing-setting variants', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'compose.prod.yml', [
      'services:',
      '  app:',
      '    image: app:latest',
    ].join('\n'))
    // file without 'prod' in filename (filtered out)
    writeFixture(cwd, 'docker-compose.dev.yml', [
      'services:',
      '  hasura:',
      '    image: hasura/graphql-engine:v2',
    ].join('\n'))
...
describe('no-non-null-assertions edge cases', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/template.ts', [
    ...
    writeFixture(cwd, 'src/comparisons.ts', [
      'export function eq(a: any, b: any) { return a !== b; }',
      'export function neq(a: any, b: any) { return a != b; }',
    ].join('\n'))
    // Test file — skipped
    writeFixture(cwd, 'src/__tests__/foo.test.ts', [
      'import { it } from "vitest";',
      'it("a", () => { const x: any = obj!.foo; });',
    ].join('\n'))
  })
