...
describe('analyzeSkippedTests (pure detector)', () => {
  ...
  it('flags fit() / fdescribe() / xit() / xdescribe()', () => {
    const src = ['fit("a", () => {})', 'fdescribe("b", () => {})', 'xit("c", () => {})', 'xdescribe("d", () => {})'].join('\n')
  ...
  it('does NOT flag a healthy JS/TS test', () => {
    const v = analyzeSkippedTests("it('renders', () => {})\ndescribe('suite', () => {})", 'a.test.ts')
    expect(v.length).toBe(0)
  })

  it('does NOT flag bare identifiers like skipped/onlyChild', () => {
    const v = analyzeSkippedTests('const skipped = 1\nconst onlyChild = 2\nlet describeIt = 3', 'a.test.ts')
    expect(v.length).toBe(0)
  })

  it('flags Python pytest/unittest skip idioms', () => {
    const src = ['@pytest.mark.skip', '@pytest.mark.skipif(True)', '@unittest.skip("x")', '@unittest.expectedFailure', 'self.skipTest("x")', 'pytest.skip("x")'].join('\n')
...
describe('noSkippedTests (wired check)', () => {
  ...
  async function findingCount(name: string, content: string): Promise<number> {
    ...
    const result = await withScope(makeTestScope(), () => noSkippedTests.run(root, { targetFiles: [filePath] }))
    return result.signals.filter((s) => s.ruleId === 'fit:no-skipped-tests').length
  }

  it('flags .only in a test file', async () => {
    expect(await findingCount('widget.test.ts', "describe.only('w', () => {})")).toBeGreaterThanOrEqual(1)
  })
