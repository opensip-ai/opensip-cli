...
describe('dangerous-config-defaults extractors', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/config.ts', [
      'export const db = {',
      '  ssl: false,',
...
describe('exit-code-correctness catch blocks', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'cli/silent.ts', [
      'export async function main() {',
      '  try {',
      '    await doWork()',
      '  } catch (err) {',
    ...
    writeFixture(cwd, 'cli/run.test.ts', [
      'export async function main() {',
    ...
    writeFixture(cwd, 'src/util.ts', [
      'export function f() { try { g() } catch (e) { logger.error(e) } }',
    ].join('\n'))
  })
