...
describe('cache-ttl-validation', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/cache/short.ts', [
      'const cacheConfig = {',
      '  ttl: 1,', // too short
      '};',
      'export { cacheConfig };',
    ].join('\n'))
    writeFixture(cwd, 'src/cache/financial.ts', [
      'const cacheConfig = {',
    ...
    writeFixture(cwd, 'src/cache/sensitive.ts', [
      'const cacheConfig = {',
    ...
    writeFixture(cwd, 'src/cache/general-too-long.ts', [
      'const cacheConfig = {',
      '  ttl: 999999, // generic cache',
      '};',
      'export { cacheConfig };',
    ].join('\n'))
    writeFixture(cwd, 'src/cache/safe.ts', [
      'const cacheConfig = {',
      '  ttl: 300,',
      '};',
      'export { cacheConfig };',
    ...
    writeFixture(cwd, 'src/cache/map-set.ts', [
      'const cache = new Map();',
...
describe('test-file-naming', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'packages/a/__tests__/foo-spec.ts', [
      'import { it } from "vitest";',
      'it("works", () => undefined);',
    ].join('\n'))
    writeFixture(cwd, 'packages/a/__tests__/bar.test.ts', [
      'import { it } from "vitest";',
      'it("works", () => undefined);',
    ].join('\n'))
    writeFixture(cwd, 'packages/a/__tests__/setup.ts', [
...
describe('test-file-pairing', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'package.json', JSON.stringify({ name: 'root' }, null, 2))
    writeFixture(cwd, 'packages/a/src/orphan.ts', [
      'export function unused() { return 1; }',
    ].join('\n'))
    writeFixture(cwd, 'packages/a/src/paired.ts', [
      'export function paired() { return 1; }',
    ].join('\n'))
    writeFixture(cwd, 'packages/a/src/__tests__/paired.test.ts', [
      'import { it, expect } from "vitest";',
      'it("works", () => expect(1).toBe(1));',
    ].join('\n'))
    writeFixture(cwd, 'packages/a/src/types.ts', [
      'export type Thing = { id: string };',
    ].join('\n'))
    writeFixture(cwd, 'packages/a/src/with-pending.ts', [
      '// @test-pending — work in progress',
      'export function pending() { return 1; }',
    ].join('\n'))
  })
...
describe('retry-config-validation edges', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/skip-comment.ts', [
    ...
    writeFixture(cwd, 'src/aggressive.ts', [
      'export const retryConfig = {',
      '  maxRetries: 3,',
      '  baseDelay: 5,',
      '};',
...
describe('no-hardcoded-timeouts edges', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/comment-line.ts', [
      '// timeout = 30000 — commented config example',
      'export const x = 1;',
    ].join('\n'))
    writeFixture(cwd, 'src/non-numeric.ts', [
      'export function setup(client: any) {',
      '  client.timeout(THIRTY_SECONDS);',
      '}',
    ].join('\n'))
  })
...
describe('dangerous-config-defaults additional', () => {
  ...
  beforeAll(() => {
    ...
    // Pool size 2
    writeFixture(cwd, 'src/pool.ts', [
      'export const config = {',
      '  poolSize: 2,',
      '};',
    ].join('\n'))
    // SSL true is OK
    writeFixture(cwd, 'src/ssl-true.ts', [
      'export const config = {',
      '  ssl: true,',
      '};',
    ].join('\n'))
...
describe('reentrancy-guard variants', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/locked.ts', [
    ...
    writeFixture(cwd, 'src/initialized.ts', [
...
describe('no-process-exit-in-finally', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/exit-in-finally.ts', [
...
describe('recovery-patterns additional', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/exponential.ts', [
...
describe('performance-anti-patterns variants', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/sync-await.ts', [
    ...
    writeFixture(cwd, 'src/parallel-ok.ts', [
      'export async function listItems(items: number[]) {',
...
describe('no-non-null-assertions variants', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/multi.ts', [
      'export function chain(arr: any[]) {',
      '  const a = arr[0]!.b!.c!;',
      '  return a;',
      '}',
    ].join('\n'))
    writeFixture(cwd, 'src/test-skip.test.ts', [
      'import { it, expect } from "vitest";',
...
describe('directive-audit suppression categories', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/many-fitness.ts', [
...
describe('dependency-vulnerability-audit', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/cookies.ts', [
...
describe('heavy-import-detection variants', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/full-lodash.ts', [
      'import _ from "lodash";',
      'export const x = _.pick({ a: 1 }, ["a"]);',
    ].join('\n'))
    writeFixture(cwd, 'src/cherry-lodash.ts', [
      'import pick from "lodash/pick";',
      'export const x = pick({ a: 1 }, ["a"]);',
    ].join('\n'))
  })
...
describe('graphql-offset-pagination variants', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(cwd, 'src/backtick.ts', [
      'export const Q = `',
    ...
    writeFixture(cwd, 'src/no-template.ts', [
      'export const Q = "no template";',
    ].join('\n'))
  })
