...
beforeAll(async () => {
  ...
  allFixturePaths = [
    ...
    fixture(
      ...
      ['export function log(msg: string) { console.error(msg); }'].join('\n'),
    ),
    fixture(
      'src/cache/store.ts',
      ['const ttl = 60000;', 'export const cacheTtl = ttl;'].join('\n'),
    ),
    fixture(
      'src/errors/index.ts',
      [
    ...
    fixture(
      'src/directives/ts-suppressions.ts',
      [
    ...
    fixture(
      ...
      'export const db = { users: { findMany: async () => [] } as any, posts: { findMany: async () => [] } as any };',
    ),
    ...
    fixture(
      'packages/inner-a/package.json',
      JSON.stringify({ name: 'inner-a', version: '1.0.0' }, null, 2),
    ),
    fixture(
      'packages/inner-b/package.json',
      JSON.stringify({ name: 'inner-b', version: '1.0.0' }, null, 2),
    ),
    ...
    // --- TEST CONVENTION ---
    fixture(
      'src/__tests__/skipped-suite.test.ts',
      [
        'import { describe, it } from "vitest";',
    ...
    fixture(
      'src/__tests__/focused-suite.test.ts',
      [
        'import { describe, it } from "vitest";',
    ...
    fixture(
      'src/schemas/no-type-constraint.ts',
      [
        'import { z } from "zod";',
    ...
    fixture('Dockerfile.api', ['FROM node:20', 'COPY . .', 'RUN npm install'].join('\n')),
    fixture('Dockerfile.web', ['FROM node:18', 'COPY . .'].join('\n')),

    // --- HEAVY-IMPORT detection (e.g. importing all of moment / lodash) ---
    fixture(
      'src/util/heavy-import.ts',
      [
