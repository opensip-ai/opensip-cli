...
/**
 * @fileoverview Behavior fixtures for remaining check scenarios.
 */
...
describe('directive-audit pure-fitness file', () => {
  ...
  it('runs without throwing on mixed directive file', async () => {
    const result = await findCheck('directive-audit').run(cwd, {
      targetFiles: [join(cwd, 'src/fitness-only.ts'), join(cwd, 'src/many-mixed.ts')],
    });
    expect(result).toBeDefined();
  });
});
...
// =============================================================================
// no-skipped-tests: cover focused (.only) + skipped (.skip) variants
// (no-focused-tests was folded into no-skipped-tests — it now detects both)
// =============================================================================

describe('no-skipped-tests variants', () => {
  let cwd: string;
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'src/__tests__/with-only.test.ts',
      [
    ...
    writeFixture(
      cwd,
      'src/__tests__/with-skip.test.ts',
      [
        'import { it, describe } from "vitest";',
  ...
  it('flags .only (focused) as a violation', async () => {
  ...
  it('flags .skip as a violation', async () => {
    const result = await findCheck('no-skipped-tests').run(cwd, {
...
describe('auth-route-guard variants', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'app/(auth)/_layout.unprotected.tsx',
...
describe('async-state-pattern skip paths', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'src/screens/__tests__/foo.test.tsx',
      [
    ...
    writeFixture(
      cwd,
      'src/screens/components/patterns/Loading.tsx',
      [
        'import { useQuery } from "@tanstack/react-query";',
...
describe('no-duplicate-packages other categories', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      ...
      JSON.stringify({ name: '@org/api-client' }, null, 2),
    );
    writeFixture(
      cwd,
      'packages/http/package.json',
      JSON.stringify({ name: '@org/http-client' }, null, 2),
    );
    writeFixture(cwd, 'packages/c1/package.json', JSON.stringify({ name: '@org/config' }, null, 2));
    writeFixture(
      cwd,
      'packages/c2/package.json',
      JSON.stringify({ name: '@org/settings' }, null, 2),
    );
  });
...
describe('docker-version-sync hardcoded pnpm', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'Dockerfile.match-but-hardcoded',
      ['FROM node:20', 'RUN corepack prepare pnpm@10.0.0 --activate'].join('\n'),
    );
  });
