...
describe('transaction-boundary-validation', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/uncommitted.ts', [
  ...
  it('flags an uncommitted transaction', async () => {
    const result = await findCheck('transaction-boundary-validation').run(cwd, { targetFiles: files })
...
describe('transaction-timeout', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/no-timeout.ts', [
...
describe('readline-cleanup', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      ...
      writeFixture(cwd, 'src/helper-defn.ts', [
        'export async function readLine(prompt: string) { return prompt; }',
      ].join('\n')),
    )
  })
...
describe('sentry-error-boundary', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      ...
      writeFixture(cwd, 'src/NoSentry.tsx', [
        'import React from "react";',
...
describe('sentry-sample-rate', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/init-too-high.ts', [
        'import * as Sentry from "@sentry/node";',
        'Sentry.init({',
        '  dsn: "https://example",',
...
describe('sentry-dsn-configured', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/init-no-dsn.ts', [
        'import * as Sentry from "@sentry/node";',
        'Sentry.init({',
        '  tracesSampleRate: 0.1,',
        '});',
...
describe('expo-vector-icons', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/Bad.tsx', [
        'import Icon from "react-native-vector-icons/FontAwesome";',
        'export const Foo = () => <Icon name="rocket" />;',
      ].join('\n')),
      writeFixture(cwd, 'src/AlsoBad.tsx', [
        'import { FaBeer } from "react-icons/fa";',
        'export const Bar = () => <FaBeer />;',
      ].join('\n')),
...
describe('async-state-pattern', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/screens/UserScreen.tsx', [
...
describe('no-raw-regex-on-code', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'fitness/src/checks/missing-filter.ts', [
      ...
      writeFixture(cwd, 'src/not-a-check.ts', [
        'export function find(content: string) {',
        '  return /foo/.test(content);',
        '}',
      ].join('\n')),
    )
  })
...
describe('hasura-production-config', () => {
  ...
  it('flags missing/incorrect Hasura security settings in production compose', async () => {
    const result = await findCheck('hasura-production-config').run(cwd, {
      targetFiles: [
        join(cwd, 'docker-compose.prod.yaml'),
        join(cwd, 'docker-compose.dev.yaml'),
      ],
    })
...
describe('auth-route-guard', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'app/(auth)/_layout.tsx', [
      ...
      writeFixture(cwd, 'app/(public)/_layout.tsx', [
        'export default function Layout() { return <Slot />; }',
      ].join('\n')),
    )
  })
...
describe('no-markdown-references', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/with-stale-ref.ts', [
...
describe('typescript-directive-hygiene', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/no-just.ts', [
        '// @ts-expect-error',
        'const x: number = "y" as any;',
      ].join('\n')),
      writeFixture(cwd, 'src/generic-just.ts', [
...
describe('graphql-offset-pagination', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/uses-offset.ts', [
...
describe('fitness-ignore-hygiene', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/no-reason.ts', [
        '// @fitness-ignore-file no-console-log',
        'console.log("hi");',
      ].join('\n')),
      writeFixture(cwd, 'src/bad-slug.ts', [
        '// @fitness-ignore-file Bad_Slug -- some reason',
        'export const x = 1;',
      ].join('\n')),
      writeFixture(cwd, 'src/excessive.ts', [
...
describe('pino-serializer-coverage', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/logs-req.ts', [
...
describe('auth-middleware-coverage', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/routes/exposed.ts', [
      ...
      writeFixture(cwd, 'src/routes/global-auth.ts', [
        'app.register(authPlugin);',
        'fastify.get("/anything", async () => 1);',
      ].join('\n')),
...
describe('api-key-rotation', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/single-equality.ts', [
        'export function check(key: string) {',
        '  return key === process.env.API_KEY;',
        '}',
      ].join('\n')),
      writeFixture(cwd, 'src/single-assignment.ts', [
...
describe('rate-limit-coverage', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/api-routes.ts', [
...
describe('cors-configuration', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/wildcard.ts', [
        'import cors from "cors";',
        'export const c = cors({ origin: "*" });',
      ].join('\n')),
      writeFixture(cwd, 'src/reflecting.ts', [
        'import cors from "cors";',
        'export const c = cors({ origin: request.headers.origin });',
      ].join('\n')),
      writeFixture(cwd, 'src/origin-true.ts', [
        'import cors from "cors";',
        'export const c = cors({ origin: true });',
      ].join('\n')),
...
describe('use-centralized-crypto', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/services/hasher.ts', [
...
describe('exit-code-correctness', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/cli/cmd.ts', [
        'export async function run() {',
        '  try { await doWork(); }',
...
describe('event-architecture', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/domain/raw-emit.ts', [
...
describe('event-handler-idempotency', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/handler-no-idem.ts', [
      ...
      writeFixture(cwd, 'src/no-state.ts', [
        'export class OrderReader { @EventHandler handle() { return 1; } }',
      ].join('\n')),
    )
  })
...
describe('navigation-typing', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(cwd, 'src/screens/Untyped.tsx', [
