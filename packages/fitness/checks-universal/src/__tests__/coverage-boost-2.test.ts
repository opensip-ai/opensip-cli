...
describe('no-hardcoded-timeouts', () => {
  ...
  beforeAll(() => {
    ...
    files.push(
      writeFixture(
        cwd,
        'src/raw-set-timeout.ts',
        ['export function delay(cb: () => void) {', '  setTimeout(cb, 30000);', '}'].join('\n'),
      ),
      writeFixture(
        cwd,
        'src/timeout-assign.ts',
        ['export const config = {', '  timeout: 60000,', '};'].join('\n'),
      ),
      writeFixture(
        cwd,
        'src/dot-timeout.ts',
        ['export function setUp(client: any) {', '  client.timeout(45000);', '}'].join('\n'),
      ),
      writeFixture(
        cwd,
        'src/short-timeout.ts',
        ['export function delay(cb: () => void) { setTimeout(cb, 1000); }'].join('\n'),
      ),
      writeFixture(
        cwd,
        'src/__tests__/skip.test.ts',
        ['it("times out", () => { setTimeout(() => {}, 30000); });'].join('\n'),
      ),
    );
  });
...
describe('error-code-registration', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      ...
      ['export const ERROR_CODES = {', '  FOO_BAR_BAZ: "FOO.BAR.BAZ",', '} as const;'].join('\n'),
    );
    writeFixture(
      cwd,
      'src/services/foo.ts',
    ...
    writeFixture(
      cwd,
      'src/services/registered.ts',
      ['export function good() {', '  throw { code: "FOO.BAR.BAZ", message: "boom" };', '}'].join(
        '\n',
      ),
    );
  });
...
describe('no-stub-tests', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      ...
      ['import { it } from "vitest";', 'it("future work", () => { /* TODO implement */ });'].join(
        '\n',
      ),
    );
    writeFixture(
      cwd,
      'src/__tests__/trivial.test.ts',
      [
...
describe('empty-package-detection', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'packages/commented/src/index.ts',
      [
        'export const a = 1',
        '// export const b = 2',
        '// export const c = 3',
...
describe('docker-version-sync', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'Dockerfile.bad',
      ['FROM node:18', 'RUN corepack prepare pnpm@9.0.0 --activate'].join('\n'),
    );
    writeFixture(
      cwd,
      'Dockerfile.good',
...
describe('docker-best-practices', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'Dockerfile.proper',
      ['FROM node:20', 'COPY . .', 'USER node', 'CMD ["node", "src/app.js"]'].join('\n'),
    );
  });
...
describe('timer-lifecycle', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'src/no-cleanup.ts',
      ['const handle = setInterval(() => {}, 60000);', 'export const stop = () => undefined;'].join(
        '\n',
      ),
    );
    writeFixture(
...
describe('graceful-shutdown', () => {
  ...
  beforeAll(() => {
    ...
    writeFixture(
      cwd,
      'src/server-no-handlers.ts',
      ['import express from "express";', 'const app = express();', 'app.listen(3000);'].join('\n'),
    );
    writeFixture(
      cwd,
      'src/server-with-handlers.ts',
      [
