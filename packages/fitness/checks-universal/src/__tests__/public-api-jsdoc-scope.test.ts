...
describe('public-api-jsdoc — package.json exports scoping', () => {
  ...
  beforeAll(() => {
    ...
    publicFile = writeFixture(cwd, 'src/public.ts', [
      'export function publicNoJsdoc() { return 1 }',
    ].join('\n'))
    internalFile = writeFixture(cwd, 'src/internal-helper.ts', [
      'export function helper() { return 2 }',
    ].join('\n'))
    // A completely unrelated file in src/ — not reachable from index.
    untrackedFile = writeFixture(cwd, 'src/orphan.ts', [
...
describe('public-api-jsdoc — fallback behavior', () => {
  ...
  beforeAll(() => {
    ...
    file = writeFixture(cwd, 'src/leaf.ts', [
...
describe('public-api-jsdoc — exports map shapes', () => {
  ...
  beforeAll(() => {
    ...
    publicFile = writeFixture(cwd, 'src/a.ts', [
      'export function aNoJsdoc() { return 1 }',
    ].join('\n'))
    writeFixture(cwd, 'src/sub.ts', [
      'export function subSymbol() { return 2 }',
    ].join('\n'))
  })
...
describe('public-api-jsdoc — binary-only package has empty surface', () => {
  ...
  beforeAll(() => {
    ...
    file = writeFixture(cwd, 'src/leaf.ts', [
      'export function leafSymbol() { return 1 }',
    ].join('\n'))
  })
...
describe('public-api-jsdoc — fallback when no exports field', () => {
  ...
  beforeAll(() => {
    ...
    file = writeFixture(cwd, 'src/public.ts', [
      'export function publicMainOnly() { return 1 }',
    ].join('\n'))
  })
