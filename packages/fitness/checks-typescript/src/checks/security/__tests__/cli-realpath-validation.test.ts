...
describe('cli-realpath-validation', () => {
  ...
  it('flags all conventional root names', () => {
    for (const name of [
      ...
      expect(analyzeCliRealpathValidation(src, CLI_FILE).length).toBeGreaterThanOrEqual(1);
    }
  });
  ...
  it('skips test files', () => {
    const src = `x.startsWith(projectRoot)\n`;
    expect(analyzeCliRealpathValidation(src, 'packages/cli/src/__tests__/x.test.ts')).toHaveLength(
      0,
    );
  });
