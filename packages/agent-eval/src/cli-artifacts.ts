...
/**
 * Remove the JSON half of a failed pair write.
 *
 * @throws {CliHarnessError} When cleanup cannot remove the partial artifact.
 */
async function removePartialJson(path: string, fileSystem: ArtifactFileSystem): Promise<void> {
  try {
    await fileSystem.remove(path);
  } catch {
    throw new CliHarnessError(
      'A report pair write failed and its partial JSON could not be removed.',
    );
  }
}
...
async function writeExclusivePair(
  ...
  try {
    ...
    if (jsonWritten) await removePartialJson(paths.jsonPath, fileSystem);
    throw error;
  }
}
...
async function writeExplicitReport(
  ...
  try {
    ...
    if (error instanceof CliHarnessError) throw error;
    throw new CliHarnessError('The report pair could not be written; check output permissions.');
  }
}
