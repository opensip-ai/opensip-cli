...
function applyOperation(workspaceRoot: string, operation: EditOperation): AppliedEditRecord {
...
export function applyEditScript(
  ...
  return script.operations.map((operation) => applyOperation(workspaceRoot, operation));
}
