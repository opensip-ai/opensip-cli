...
describe('graph:high-blast-untested shipped defaults (warn 75 / error 150)', () => {
  it('is silent below 75, medium in [75, 150), high at/above 150', () => {
    ...
    expect(run({ bodyLines: 5, blast: blast(75), testReachable: false }, EMPTY)[0]?.severity).toBe('medium');
    expect(run({ bodyLines: 5, blast: blast(149), testReachable: false }, EMPTY)[0]?.severity).toBe('medium');
    expect(run({ bodyLines: 5, blast: blast(150), testReachable: false }, EMPTY)[0]?.severity).toBe('high');
  });
});
