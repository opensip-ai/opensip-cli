...
describe('resolveCallee', () => {
  ...
  const bG = occ({ bodyHash: 'G', filePath: 'packages/pkg-b/src/g.ts', qualifiedName: 'packages/pkg-b/src/g.g', simpleName: 'g' });
  ...
  it('prefers the caller’s own package on a cross-package body collision', () => {
    const caller = occ({ bodyHash: 'C', filePath: 'packages/pkg-a/src/call.ts', qualifiedName: 'packages/pkg-a/src/call.caller', simpleName: 'caller' });
  ...
  it('falls back to an imported package when no same-package candidate exists', () => {
    const caller = occ({ bodyHash: 'C', filePath: 'packages/pkg-c/src/call.ts', qualifiedName: 'packages/pkg-c/src/call.caller', simpleName: 'caller' });
...
describe('callerImportedPackages', () => {
  it('reads the import set for the caller’s file, empty when absent (fast mode)', () => {
    const caller = occ({ bodyHash: 'C', filePath: 'packages/pkg-a/src/x.ts', qualifiedName: 'packages/pkg-a/src/x.c', simpleName: 'c' });
    const withImports = indexesOf([caller], new Map([['packages/pkg-a/src/x.ts', new Set(['core'])]]));
