...
function occ(over: Partial<FunctionOccurrence> & { bodyHash: string; simpleName: string }): FunctionOccurrence {
  return {
...
describe('noSideEffectPathRule', () => {
  ...
  it('suppresses the signal when a transitive callee has a side effect (adapter primitives)', () => {
    ...
    const catalog = catalogOf([
      occ({ bodyHash: 'PURE', simpleName: 'computeThing', endLine: 20, calls: [edge('LEAF1'), edge('LEAF2')] }),
      occ({ bodyHash: 'LEAF1', simpleName: 'leaf1', visibility: 'module-local', calls: [edge('X', { text: 'print(x)' })] }),
      occ({ bodyHash: 'LEAF2', simpleName: 'leaf2', visibility: 'module-local' }),
      occ({ bodyHash: 'CALLER', simpleName: 'driver', visibility: 'module-local', calls: [edge('PURE', { discarded: true })] }),
    ]);
    const hints: RuleHints = { sideEffectPrimitives: ['print', 'os.system'] };
    const signals = noSideEffectPathRule.evaluate(catalog, buildIndexes(catalog), EMPTY_CONFIG, hints);
  ...
  it('does not flag when no caller discards the return value', () => {
    const catalog = catalogOf([
      ...
      occ({ bodyHash: 'LEAF2', simpleName: 'leaf2', visibility: 'module-local' }),
      occ({ bodyHash: 'CALLER', simpleName: 'driver', visibility: 'module-local', calls: [edge('PURE', { discarded: false })] }),
    ]);
    const signals = noSideEffectPathRule.evaluate(catalog, buildIndexes(catalog), EMPTY_CONFIG, undefined);
    expect(signals).toEqual([]);
  });
...
describe('orphanSubtreeRule', () => {
  it('flags a module-local function unreachable from any entry point', () => {
    ...
    const signals = orphanSubtreeRule.evaluate(catalog, buildIndexes(catalog), EMPTY_CONFIG, undefined);
  ...
  it('treats a config-seeded entryPointHash as reachable, suppressing its orphan signal', () => {
    const orphan = occ({ bodyHash: 'SEEDED', simpleName: 'seededHelper', visibility: 'module-local' });
...
describe('testOnlyReachableRule', () => {
  it('flags a module-local function whose only caller is a test file', () => {
    ...
    const prodReached = occ({ bodyHash: 'PRODREACHED', simpleName: 'prodReached', visibility: 'module-local' });
    const testFixture = occ({ bodyHash: 'FIXTURE', simpleName: 'testFixture', visibility: 'module-local' });
    ...
    const signals = testOnlyReachableRule.evaluate(catalog, buildIndexes(catalog), EMPTY_CONFIG, undefined);
