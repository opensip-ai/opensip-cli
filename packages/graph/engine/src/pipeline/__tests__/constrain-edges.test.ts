...
describe('constrainCrossPackageEdges', () => {
  it('drops a name-guessed edge into a package the caller does not import', () => {
    ...
    const caller = occ({ bodyHash: 'C', filePath: A, package: PKGA, simpleName: 'caller', calls: [edge({ to: ['H'] })] });
  ...
  it('keeps a name-guessed edge into a package the caller imports (by specifier)', () => {
    ...
    const out = constrainCrossPackageEdges(catalogOf({ f: [target], caller: [caller], '<module-init>': [mi] }));
  ...
  it('never drops a type-checker-backed (static) edge, even cross-package', () => {
    const target = occ({ bodyHash: 'H', filePath: 'packages/pkg-b/src/f.ts', package: PKGB });
    const caller = occ({ bodyHash: 'C', filePath: A, package: PKGA, simpleName: 'caller', calls: [edge({ to: ['H'], resolution: 'static', confidence: 'high' })] });
    const out = constrainCrossPackageEdges(catalogOf({ f: [target], caller: [caller] }));
    expect(out.functions.caller[0].calls[0].to).toEqual(['H']);
  ...
  it('keeps a colliding hash when any occurrence is in a reachable package', () => {
    const aF = occ({ bodyHash: 'H', filePath: 'packages/pkg-a/src/f.ts', package: PKGA, qualifiedName: 'a.f' });
    const bF = occ({ bodyHash: 'H', filePath: 'packages/pkg-b/src/f.ts', package: PKGB, qualifiedName: 'b.f' });
    const caller = occ({ bodyHash: 'C', filePath: A, package: PKGA, simpleName: 'caller', calls: [edge({ to: ['H'] })] });
  ...
  it('drops only the unreachable targets of a multi-target edge', () => {
    const bF = occ({ bodyHash: 'Hb', filePath: 'packages/pkg-b/src/f.ts', package: PKGB, qualifiedName: 'b.f' });
    const cF = occ({ bodyHash: 'Hc', filePath: 'packages/pkg-c/src/f.ts', package: PKGC, qualifiedName: 'c.f' });
    const caller = occ({ bodyHash: 'C', filePath: A, package: PKGA, simpleName: 'caller', calls: [edge({ to: ['Hb', 'Hc'] })] });
    const mi = moduleInit(A, PKGA, [PKGC]); // imports pkg-c only
    const out = constrainCrossPackageEdges(catalogOf({ f: [bF, cF], caller: [caller], '<module-init>': [mi] }));
  ...
  it('drops a name-guessed edge into another package’s test file (builtin .map/.find collision)', () => {
    const testArrow = occ({ bodyHash: 'H', filePath: 'packages/pkg-b/src/__tests__/x.test.ts', package: PKGB, simpleName: 'map', inTestFile: true });
    const caller = occ({ bodyHash: 'C', filePath: A, package: PKGA, simpleName: 'caller', calls: [edge({ to: ['H'], text: 'xs.map(...)' })] });
    const mi = moduleInit(A, PKGA, [PKGB]); // pkg-a DOES import pkg-b
    const out = constrainCrossPackageEdges(catalogOf({ f: [testArrow], caller: [caller], '<module-init>': [mi] }));
  ...
  it('keeps a same-package test-file callee (test calling its own package’s test helper)', () => {
    const testHelper = occ({ bodyHash: 'H', filePath: 'packages/pkg-a/src/__tests__/helper.test.ts', package: PKGA, inTestFile: true });
    const caller = occ({ bodyHash: 'C', filePath: 'packages/pkg-a/src/__tests__/x.test.ts', package: PKGA, simpleName: 'caller', inTestFile: true, calls: [edge({ to: ['H'] })] });
  ...
  it('ignores relative and external specifiers, unions multiple workspace imports', () => {
    const bF = occ({ bodyHash: 'Hb', filePath: 'packages/pkg-b/src/f.ts', package: PKGB, qualifiedName: 'b.f' });
    const cF = occ({ bodyHash: 'Hc', filePath: 'packages/pkg-c/src/f.ts', package: PKGC, qualifiedName: 'c.f' });
    const callB = occ({ bodyHash: 'CB', filePath: A, package: PKGA, simpleName: 'callB', calls: [edge({ to: ['Hb'] })] });
    const callC = occ({ bodyHash: 'CC', filePath: A, package: PKGA, simpleName: 'callC', calls: [edge({ to: ['Hc'] })] });
