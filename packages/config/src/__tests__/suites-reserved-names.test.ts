...
describe('reserved suite names (ADR-0159)', () => {
  it('rejects a configured suite using a reserved built-in name', () => {
    const result = suitesConfigSchema.safeParse({
      audit: { steps: [VALID_STEP] },
    });
    expect(result.success).toBe(false);
    const issue = result.success ? undefined : result.error.issues[0];
    expect(issue?.path).toEqual(['audit']);
    expect(issue?.message).toContain("Suite name 'audit' is reserved");
    expect(issue?.message).toContain('opensip suite run audit-custom');
  });
  ...
  it('exposes the reserved list and membership helper coherently', () => {
    expect(RESERVED_SUITE_NAMES).toContain('audit');
