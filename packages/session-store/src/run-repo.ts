...
const EMPTY_RUN_RETENTION_RESULT: RunRetentionBatchResult = Object.freeze({
  examined: 0,
  protected: 0,
  deleted: 0,
  remainingEligible: 0,
...
function requireRetentionBatchSize(value: number): number {
...
function countRunRows(tx: DrizzleHandle): number {
  return tx.select({ value: count() }).from(runs).get()?.value ?? 0;
}

function pruneCountExcessRunBatch(
  tx: DrizzleHandle,
  keep: number,
  batchSize: number,
): RunRetentionBatchResult {
