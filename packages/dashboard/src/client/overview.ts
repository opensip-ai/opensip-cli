...
function appendLegacySessionRows(tbody: HTMLElement, ledgerRows: LedgerRenderedRows): void {
  ...
  for (const session of sessions) {
    ...
    if (shouldSkipLegacySuite(session.suiteRunId, ledgerRows.suiteRunIds, renderedSuiteRunIds)) {
      continue;
    }
...
function shouldSkipLegacySuite(
  ...
  return ledgerSuiteRunIds.has(suiteRunId) || renderedSuiteRunIds.has(suiteRunId);
}

function appendRecentActivityRows(tbody: HTMLElement): void {
  appendLegacySessionRows(
    tbody,
