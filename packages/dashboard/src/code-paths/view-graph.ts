...
views.push({
  ...
  render(container, catalog, indexes, filterState) {
    ...
    // Node selection — toggle a CSS class for now (later phases add impact).
    gvCy.on('tap', 'node', function(evt) {
      gvCy.nodes().removeClass('gv-node-selected');
      evt.target.addClass('gv-node-selected');
    });
    gvCy.on('tap', function(evt) {
      if (evt.target === gvCy) gvCy.nodes().removeClass('gv-node-selected');
    });
  },
  onActivate() {
