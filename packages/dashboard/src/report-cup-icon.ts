...
const REPORT_CUP_BRAND = '#C8956C';
const REPORT_CUP_STEAM = '#e8e4df';
const REPORT_CUP_LID = '#d8d4ce';

/** Geometric favicon geometry (32×32 viewBox), matching opensip.ai/favicon.svg. */
const REPORT_CUP_SVG_INNER =
  `<circle cx="11" cy="3" r="1.8" fill="${REPORT_CUP_STEAM}"/>` +
  `<circle cx="20" cy="3" r="1.8" fill="${REPORT_CUP_STEAM}"/>` +
  `<rect x="3" y="7" width="26" height="8" rx="1" fill="${REPORT_CUP_LID}"/>` +
  `<rect x="3" y="16" width="26" height="9" fill="${REPORT_CUP_BRAND}"/>` +
  `<rect x="7" y="26" width="18" height="4" rx="1" fill="${REPORT_CUP_BRAND}"/>`;
...
export const REPORT_CUP_HEADER_HTML = `<pre aria-hidden="true" style="${MINI_CUP_PRE_STYLE}">${MINI_CUP.map(
  (line, i) =>
    `<span style="color:${MINI_CUP_LIGHT_ROWS.has(i) ? MINI_CUP_LIGHT : REPORT_CUP_BRAND};display:block;${
