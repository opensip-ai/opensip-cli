...
import { dashboardContributedTabsJs, type ContributedTab } from './contributed-tabs.js';
...
export type { ContributedTab, ContributedView } from './contributed-tabs.js';
...
export interface DashboardInput {
  ...
  /**
   * Host-owned per-run dashboard tabs contributed by tools through the
   * `ToolDashboardContribution` seam (host-owned-run-timing Phase 5 §7). This is
   * a HOST shell key — the CLI's `report-compose` resolves each contribution's
   * tab + inline rows into this list (namespaced by tool id), and the dashboard
   * renders them generically via {@link dashboardContributedTabsJs} WITHOUT
   * importing any tool package. Tools cannot set this key from
   * `collectReportData` (it is reserved alongside `sessions`).
   */
  contributedTabs?: readonly ContributedTab[];
}
...
export function generateDashboardHtml(input: DashboardInput): string {
  ...
  // Contributed per-run tabs (host-owned-run-timing Phase 5). Ordered by the
  // contribution's `order`, then title, for a stable tab-bar layout. Each gets a
  // tab button + a `panel-<id>` container; the inlined JSON + the generic
  // renderer (`renderContributedTabs`) populate them client-side. The ids are
  // already host-namespaced (e.g. `contrib-fit-fit-run-summary`) so there is no
  // collision with the registered tool tabs or the overview shell.
  const orderedContributedTabs = [...contributedTabs].sort(
    (a, b) => a.order - b.order || a.title.localeCompare(b.title),
  );
  const contributedTabButtons = orderedContributedTabs
    .map((t) => `  <div class="tab" data-tab="${t.id}">${t.title}</div>`)
    .join('\n');
  const contributedTabPanels = orderedContributedTabs
    .map((t) => `<div id="panel-${t.id}" class="tab-panel"></div>`)
    .join('\n');
  const safeContributedTabsJson = escapeForScriptContext(JSON.stringify(orderedContributedTabs));
...
const contributedTabs = ${safeContributedTabsJson};
${editorProtocolJs}
