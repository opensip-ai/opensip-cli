 ...
 * the GENERIC capability substrate (`@opensip-cli/core`) now. This module keeps
 * only the sim-side reader for the documented config keys — sim resolves its own
 * preference without depending on `@opensip-cli/config`, then hands the result
 ...
 * `plugins.packageScopes` from the project's opensip-cli.config.yml without a
