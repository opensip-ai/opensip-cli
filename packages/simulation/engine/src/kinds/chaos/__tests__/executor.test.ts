...
describe('validateChaosScenarioConfig', () => {
  ...
  it('rejects a non-function target', () => {
    expect(() => validateChaosScenarioConfig(base({ target: undefined as never }))).toThrow(/target/)
  })
  ...
  it('rejects an empty faults array', () => {
    expect(() => validateChaosScenarioConfig(base({ fault: { faults: [], probability: 0.5 } }))).toThrow(
      /faults/,
    )
  })
