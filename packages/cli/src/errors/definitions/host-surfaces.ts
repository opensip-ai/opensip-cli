...
export const hostSurfacesDefinitions = {
  ...
  /**
   * Capability worker advisory guard blocked an undeclared resource (filesystem, network,
   * subprocess). Admission is the enforced boundary; this is defense-in-depth.
   */
  'CLI.CAPABILITY.RESOURCE_DENIED': {
    ...SECURITY_REFUSAL,
    code: 'CLI.CAPABILITY.RESOURCE_DENIED',
    defaultResponsibility: 'tool-author',
    exitClass: 'plugin-incompatible',
    operatorAction:
      'The capability pack used a resource its manifest did not declare. Declare the resource or stop using it.',
    publicMetadataKeys: ['condition', 'resource'],
  },

  /**
   * Isolation was required but the owning tool exposes no capabilityIsolationBridges entry
   * for the domain.
   */
  'CLI.CAPABILITY.BRIDGE_MISSING': {
    ...TOOL_AUTHOR_CONTRACT,
    code: 'CLI.CAPABILITY.BRIDGE_MISSING',
    kind: 'not-found',
    operatorAction:
      'Register a capabilityIsolationBridges entry for this domain on the owning tool, or load the pack without worker isolation.',
    publicMetadataKeys: ['condition', 'domainId', 'tool'],
  },
