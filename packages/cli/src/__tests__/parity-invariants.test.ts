  ...
  it('every named enforcement check resolves to a registered fitness check', () => {
    for (const [invariant, slugs] of Object.entries(INVARIANT_CHECKS)) {
      for (const slug of slugs) {
        expect(
          ALL_SLUGS.has(slug),
          `the parity-invariant index names check '${slug}' (for '${invariant}'), but no registered check has that slug — update and this map together`,
        ).toBe(true);
      }
    }
  });
