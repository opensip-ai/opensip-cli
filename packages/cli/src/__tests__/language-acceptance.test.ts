...
describe.each(LANGS)("language acceptance: $lang", (row) => {
  it("fit --json runs with no adapter-load error", () => {
    ...
    expect(
      res.exitCode,
      `fit exited ${res.exitCode}; stderr: ${res.stderr}`,
    ).toBe(0);
    for (const marker of [
      "plugin failed to load",
      "lang plugin failed to load",
    ]) {
  ...
  if (row.lang === "cpp") {
    ...
    it("C++ adapter discovers .cpp files (smoke)", () => {
      ...
      const res = runInFixture(
        ["fit", "--json", "--check", "no-todo-comments"],
        "cpp",
      );
      expect(
        res.exitCode,
      ...
      const filesSeen = (parsed.units ?? []).reduce(
        (a, u) => a + (u.filesValidated ?? 0),
        0,
      );
      expect(
        filesSeen,
...
describe.each(LANGS.filter((l) => l.graph))(
  "graph acceptance: $lang",
  (row) => {
    it("graph --json yields a well-formed, non-empty envelope", () => {
      const res = runInFixture(["graph", "--json"], row.lang);
      expect(
        res.exitCode,
        `graph exited ${res.exitCode}; stderr: ${res.stderr}`,
      ).toBe(0);
