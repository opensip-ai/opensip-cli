...
// `kind: "fit-pack"` package: installed via `opensip-tools plugin add
...
//
// Authored against the installed `@opensip-tools/fitness` so it goes through
// the real `defineCheck` API (and therefore resolves the same
// `@opensip-tools/core` the engine uses — exercising the single-core path).
