...
import {
  assertIncompletePathAbsent,
  removeIncompleteBlobRoot,
  type RecoverableBlobRoot,
  type RecoverableFileAuthority,
} from './authored-state-transaction-artifact-removal.js';
import {
  encodeOwner,
  expectedBlobNames,
  ownerFor,
...
function discardIncompleteAuthority(
  ...
  if (authority.stage !== null) removeIncompleteBlobRoot(root, authority.stage);
  if (authority.backup !== null) removeIncompleteBlobRoot(root, authority.backup);
  if (authority.replay !== null) {
    ...
    assertIncompletePathAbsent(authority.replay.entry.path, INCOMPLETE_REPLAY_DESCRIPTION);
