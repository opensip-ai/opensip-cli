...
function inspectRecoverableMarker(
  ...
  if (!rootNames.delete(AUTHORED_ARTIFACT_OWNER_FILE)) {
    if (rootNames.size > 0) {
      authoredTransactionFailure(
        "an ownerless authored root has unexpected entries",
      );
    }
    return "absent";
  }
  const marker = readStableArtifactFile(
  ...
  if (
    ...
    authoredTransactionFailure(
      "an incomplete authored owner marker is not a canonical prefix",
    );
  }
  if (observedBytes !== expectedBytes) {
    if (rootNames.size > 0) {
      authoredTransactionFailure(
        "a partial authored owner marker has unexpected siblings",
      );
    }
    return "partial";
  }
...
function inspectRecoverableBlobDirectory(
  ...
  if (
    ...
    authoredTransactionFailure(
      "an incomplete authored blob directory is unsafe",
    );
  }
  const expected = expectedBlobNames(manifest, kind);
  if (expected.size === 0) {
    authoredTransactionFailure(
      "an incomplete authored root has an unexpected blob directory",
    );
  }
  ...
  for (const basename of blobNames) {
    if (!expected.has(basename)) {
      authoredTransactionFailure(
        "an incomplete authored artifact has an unowned blob",
      );
    }
    ...
    if (totalBytes > INIT_AUTHORED_PLAN_CAPS.maxAggregateBlobBytes) {
      authoredTransactionFailure(
        "incomplete authored blobs exceed their recovery cap",
      );
    }
  }
  return blobNames;
...
function inspectRecoverableBlobRoot(
  ...
  const marker = inspectRecoverableMarker(
    rootPath,
    ownerFor(journal, role),
    rootNames,
  );
  if (marker !== "exact") {
  ...
  if (rootNames.size > 0) {
    authoredTransactionFailure(
      "an incomplete authored artifact has unowned entries",
    );
  }
  if (!hasBlobDirectory) {
    return { marker, hasBlobDirectory: false, blobNames: [] };
  ...
  if (manifest === null) {
    authoredTransactionFailure(
      "an incomplete authored blob directory has no replay manifest",
    );
  }
  return {
    marker,
...
function removeRecoverableBlobRoot(
  ...
  const kind = role === "stage" ? "desired" : "preimage";
  const inspected = inspectRecoverableBlobRoot(
    rootPath,
    journal,
    manifest,
    role,
...
function assertPendingPreparation(journal: RuntimePromotionJournal): void {
  if (journal.progress.pendingIntent?.kind !== "authored-prepare") {
    authoredTransactionFailure(
      "partial authored recovery requires its durable preparation intent",
    );
  }
}
...
export function validateIncompleteAuthoredArtifacts(
  ...
  if ((stagePresent || backupPresent) && !replayPresent) {
    authoredTransactionFailure(
      "authored artifacts exist without their durable replay manifest",
    );
  }
  if (replayPresent) {
    if (manifest === null || manifestBytes === null) {
      authoredTransactionFailure(
        "the durable replay manifest could not be loaded",
      );
    }
...
export function discardIncompleteAuthoredArtifacts(
  ...
  if (pathPresent(paths.replayManifest)) {
    if (manifestBytes === null) {
      authoredTransactionFailure(
        "partial replay cleanup requires its exact manifest bytes",
      );
    }
    validateReplay(journal, manifestBytes, paths.replayManifest);
    unlinkSync(paths.replayManifest);
