...
function parseOwner(raw: string): AuthoredArtifactOwner {
  ...
  try {
    ...
    authoredTransactionFailure(
      "an authored artifact owner marker is malformed",
    );
  }
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    authoredTransactionFailure(
      "an authored artifact owner marker has the wrong shape",
    );
  }
  ...
  if (encodeOwner(canonical) !== raw) {
    authoredTransactionFailure(
      "an authored artifact owner marker is noncanonical",
    );
  }
  return canonical;
}
...
export function ownerFor(
  ...
  const slot =
    role === "stage"
      ? journal.owned.authoredStage
      : journal.owned.authoredBackup;
  return {
    kind: AUTHORED_ARTIFACT_OWNER_KIND,
    version: AUTHORED_ARTIFACT_OWNER_VERSION,
...
export function authoredArtifactPaths(
  ...
  if (
    !basenames.every(isSafeRuntimePromotionBasename) ||
    new Set(basenames).size !== 3
  ) {
    authoredTransactionFailure(
      "authored artifact basenames are unsafe or collide",
    );
  }
  return {
...
export function expectedOwner(
  ...
  if (
    marker.mode !== 0o600 ||
    parseOwner(marker.bytes.toString("utf8")).ownershipId !==
      expected.ownershipId
  ) {
    authoredTransactionFailure(
      "an authored artifact owner marker does not match the journal",
    );
  }
  if (marker.bytes.toString("utf8") !== encodeOwner(expected)) {
    authoredTransactionFailure(
      "an authored artifact belongs to another operation",
    );
  }
}
...
export function expectedBlobNames(
  ...
  for (const mutation of manifest.mutations) {
    ...
    if (basename === undefined || expected.has(basename)) {
      authoredTransactionFailure(
        "the replay manifest has colliding blob references",
      );
    }
...
function assertBlobRootLayout(
  ...
  if (
    ...
    authoredTransactionFailure(
      "an authored artifact root has unexpected entries",
    );
  }
}
...
function assertBlobDirectory(rootPath: string, kind: string): void {
  ...
  if (
    ...
    authoredTransactionFailure(
      "an authored blob directory has unsafe identity",
    );
  }
}
...
function validateExpectedBlobs(
  ...
  for (const [basename, mutation] of expected) {
    ...
    if (file.mode !== state.mode || file.digest !== state.digest) {
      authoredTransactionFailure(
        "an authored blob was changed after preparation",
      );
    }
    totalBytes += file.bytes.length;
    if (totalBytes > INIT_AUTHORED_PLAN_CAPS.maxAggregateBlobBytes) {
      authoredTransactionFailure(
        "authored blob validation exceeded its aggregate cap",
      );
    }
  }
  if (observed.size > 0) {
...
export function validateAuthoredArtifacts(
  ...
  if (
    ...
    if (
      ...
      authoredTransactionFailure(
        "the stored replay manifest does not match the journal",
      );
    }
  }
  if (
  ...
  if (aggregateBytes > INIT_AUTHORED_PLAN_CAPS.maxAggregateBlobBytes) {
    authoredTransactionFailure(
      "authored artifacts exceed their aggregate blob cap",
    );
  }
  assertStableAuthoredRoot(root);
}

function authoredPlanDigest(
  manifestBytes: string,
  manifest: AuthoredReplayManifest,
): string {
  return sha256Bytes(
...
export function loadAuthoredReplayManifest(
  ...
  if (
    ...
    authoredTransactionFailure(
      "the replay manifest does not match the durable plan identity",
    );
  }
