...
function isFinalDirectionTarget(
  ...
  return (
    journal.progress.rollbackCursor + 1 === journal.progress.authoredCursor
  );
}
...
function assertPreMutationState(
  ...
  const expected =
    direction === "commit" ? mutation.preimage : mutation.desired;
  if (
    !sameAuthoredPathState(
      observeAuthoredPath(
        openStableAuthoredRoot(state.projectRoot),
        mutation.path,
      ),
      expected,
    )
    ...
    authoredTransactionFailure(
      "a target changed before its write-ahead intent",
    );
  }
}
...
async function executeAuthoredDirection(
  ...
  while (!directionDone(direction, journal, state.executionOrder.length)) {
    ...
    if (mutation === undefined)
      authoredTransactionFailure("the journal cursor is out of range");
    ...
    if (pending?.kind !== expectedKind || pending.cursor !== cursor) {
      authoredTransactionFailure(
        "the pending authored intent does not match its cursor",
      );
    }
    state.dependencies.checkpoint("before-target-mutation", cursor);
    const outcome = reconcileAuthoredMutation(
      root,
      mutation,
      state.paths,
      direction,
...
export async function rollbackAuthoredState(
  ...
  if (journal.progress.runtimeInstallState === "installed") {
    authoredTransactionFailure(
      "runtime authority must be rolled back before authored state",
    );
  }
...
export async function cleanupAuthoredState(
  ...
  if (journal.operationId !== transaction.operationId) {
    authoredTransactionFailure(
      "the cleanup receipt belongs to another operation",
    );
  }
  const root = openStableAuthoredRoot(state.projectRoot);
  for (const slot of [
    ...
    if (
      journal.cleanup[slot] === "removed" ||
      journal.cleanup[slot] === "unmaterialized"
    )
      continue;
    const pending = journal.progress.pendingIntent;
    if (pending === null) {
      state.dependencies.checkpoint("before-cleanup-intent");
      receipt = await recordCleanupIntent(
        state.controller,
        receipt,
        slot,
        state.dependencies.now,
    ...
    const manifest =
      slot === "replayManifest" && state.manifest === null
        ? null
        : manifestFor(state);
    const outcome = removeAuthoredArtifact(
      root,
      journal,
      manifest,
      state.paths,
