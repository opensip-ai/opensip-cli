...
function integer(value: unknown, field: string, maximum: number): number {
  if (
    ...
    authoredPlanFailure(
      `${field} must be an integer from 0 through ${String(maximum)}`,
    );
  }
  return value;
}
...
function parseState(value: unknown, field: string): InitAuthoredPathState {
  ...
  if (typeof object.exists !== "boolean")
    authoredPlanFailure(`${field}.exists must be boolean`);
  const type = nullableChoice(object.type, TARGET_TYPES, `${field}.type`);
  const mode =
    object.mode === null ? null : integer(object.mode, `${field}.mode`, 0o777);
  ...
  if (type === null || mode === null || hash === null) {
    authoredPlanFailure(
      `${field} existing state requires type, mode, and digest`,
    );
  }
  if ((mode & 0o022) !== 0)
    authoredPlanFailure(`${field}.mode is group/world writable`);
...
function parseBlobName(
  ...
  if (!state.exists || state.type === "directory") {
    if (value !== null)
      authoredPlanFailure(`${kind} directory/absent blob must be null`);
    return null;
  }
  ...
  if (
    ...
    Buffer.byteLength(expected, "utf8") >
      INIT_AUTHORED_PLAN_CAPS.maxBlobNameBytes
  ) {
    authoredPlanFailure(
      `${kind} blob name is not the canonical host-generated name`,
    );
  }
  return expected;
}
...
function parseMutation(value: unknown, index: number): InitAuthoredMutation {
  ...
  if (sequence !== index)
    authoredPlanFailure(`${field}.sequence is noncanonical`);
  const action = choice(object.action, ACTIONS, `${field}.action`);
  if (typeof object.path !== "string")
    authoredPlanFailure(`${field}.path must be a string`);
  const path = normalizeProjectRelativePath(object.path);
  const targetType = choice(
    object.targetType,
    TARGET_TYPES,
    `${field}.targetType`,
  );
  const targetMode = integer(object.targetMode, `${field}.targetMode`, 0o777);
  if ((targetMode & 0o022) !== 0)
    authoredPlanFailure(`${field}.targetMode is unsafe`);
  ...
  return {
    ...
    preimage,
    desired,
    desiredBlob: parseBlobName(
      object.desiredBlob,
      "desired",
      sequence,
      desired,
    ),
    preimageBlob: parseBlobName(
      object.preimageBlob,
      "preimage",
      sequence,
      preimage,
...
function parseInputs(value: unknown): InitAuthoredNormalizedInputs {
  ...
  if (!Array.isArray(object.languages))
    authoredPlanFailure("inputs.languages must be an array");
  const languages = object.languages.map((language, index) => {
    if (typeof language !== "string") {
      authoredPlanFailure(
        `inputs.languages[${String(index)}] must be a string`,
      );
    }
    return language;
  });
  ...
  if (typeof object.mode !== "string")
    authoredPlanFailure("inputs.mode must be a string");
  validateMode(object.mode);
  if (!Array.isArray(object.tools))
    authoredPlanFailure("inputs.tools must be an array");
...
function parseManifest(value: unknown): AuthoredReplayManifest {
  ...
  if (!Array.isArray(object.mutations))
    authoredPlanFailure("manifest.mutations must be an array");
  ...
  for (let index = 0; index < mutations.length; index += 1) {
    ...
    if (
      index > 0 &&
      compareUtf8(mutations[index - 1].path, mutation.path) >= 0
    ) {
      authoredPlanFailure(
        "manifest mutations are not in canonical UTF-8 path order",
      );
    }
    const caseFolded = caseFoldPath(mutation.path);
    if (folded.has(caseFolded))
      authoredPlanFailure("manifest paths collide under case folding");
...
export function encodeAuthoredReplayManifest(
  manifest: AuthoredReplayManifest,
): string {
  const canonical = parseManifest(manifest);
  const bytes = JSON.stringify(canonical);
  if (
    Buffer.byteLength(bytes, "utf8") > INIT_AUTHORED_PLAN_CAPS.maxManifestBytes
  ) {
    authoredPlanFailure(
...
export function parseAuthoredReplayManifest(
  raw: string,
): AuthoredReplayManifest {
  if (
    Buffer.byteLength(raw, "utf8") > INIT_AUTHORED_PLAN_CAPS.maxManifestBytes
  ) {
    authoredPlanFailure(
      `replay manifest exceeds ${String(INIT_AUTHORED_PLAN_CAPS.maxManifestBytes)} bytes`,
    );
