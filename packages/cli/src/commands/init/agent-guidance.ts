...
export function buildManagedAgentGuidance(): string {
  return [
    ...
    'Graph audit notes:',
    '',
    '- Every graph response includes project/catalog context, freshness verification coverage, and independent page vs coverage truncation.',
    '- Ordinary MCP graph reads auto-load a newer catalog already persisted by `opensip graph`; they never rebuild. Use `refresh_graph` only when the catalog is missing or freshness is stale/incomplete — not merely because an external graph run just finished.',
    '- Traversal defaults to occurrence identity; body-twin union is explicit. Package tools label call vs import evidence; `get_runtime_wiring` is live declarative wiring, not a static call edge.',
    '- Prefer cursors/filters/limits over unbounded walks; do not loop `refresh_graph` per query.',
    '',
