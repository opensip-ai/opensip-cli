...
const SECRET_EXAMPLE =
  "// exact-example-bytes-not-in-manifest\nexport const value = 1;\n";
...
function fixtureTool(
  ...
  return {
    ...
    ...(overrides.withExamplesHook === false
      ? {}
...
describe("Init authored replay plan", () => {
  it("plans pristine state without writing and evaluates every Tool hook once", () => {
    ...
    expect(
      mutation(plan, "opensip-cli/fit/checks/example-check-typescript.mjs")
        .action,
    ).toBe("create");
  ...
  it("refreshes only managed root documents while preserving config and authored files", () => {
    ...
    writeProjectFile(
      "opensip-cli/fit/checks/custom.mjs",
      "export default 1;\n",
    );
    ...
    expect(mutation(plan, "opensip-cli/fit/checks/custom.mjs").action).toBe(
      "preserve",
    );
    expect(mutation(plan, "AGENTS.md").action).toBe("replace");
    expect(mutation(plan, ".gitignore").action).toBe("create");
    expect(
      plan.mutations.some((entry) =>
        entry.path.includes("example-check-typescript"),
      ),
    ).toBe(false);
  });
  ...
  it("keeps custom targets and creates only missing Tool examples", () => {
    ...
    writeProjectFile(
      "opensip-cli/fit/checks/example-check-typescript.mjs",
      "// customer edited\n",
    );
    ...
    expect(
      mutation(plan, "opensip-cli/fit/recipes/example-recipe.mjs").action,
    ).toBe("create");
  ...
  it("removes the authored tree exactly while immutably excluding .runtime", () => {
    ...
    expect(
      plan.mutations.some((entry) => entry.path.includes(".runtime")),
    ).toBe(false);
    expect(
      readFileSync(
        join(projectRoot, "opensip-cli/.runtime/datastore.sqlite"),
        "utf8",
      ),
  ...
  it("stores complete replay records and rejects noncanonical or tampered manifests", () => {
    ...
    expect(() =>
      parseAuthoredReplayManifest(` ${plan.replayManifestBytes}`),
    ).toThrow(/canonical/u);

    const unknown = JSON.parse(plan.replayManifestBytes) as Record<
      string,
      unknown
    >;
    unknown.extra = true;
    ...
    expect(() => parseAuthoredReplayManifest(JSON.stringify(unsafe))).toThrow(
      /unsafe/u,
    );
  ...
  it("enforces mutation, path, collision, and runtime-target caps before returning a plan", () => {
    ...
    expect(() =>
      build("fresh", [fixtureTool({ domain: ".runtime", examples: [] })]),
    ).toThrow(/immutable/u);
  });
  ...
  it("rejects oversized, mutated, linked, special, and unsafe preimages", () => {
    ...
    expect(() =>
      build("fresh", undefined, {
        afterFileRead(path) {
          if (path === ".gitignore")
            writeFileSync(join(projectRoot, ".gitignore"), "changed\n");
        },
      }),
    ).toThrow(/changed/u);
    ...
    expect(() => build("fresh")).toThrow(
      /unsupported file type|must be a file/u,
    );
  });
  ...
  it("rejects individual files above the named cap before allocating their content", () => {
    ...
    truncateSync(
      ...
      INIT_AUTHORED_PLAN_CAPS.maxFileBytes + 1,
    );
