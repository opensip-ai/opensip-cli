...
function authoredPlan(
  specifications: readonly MutationSpec[],
): InitAuthoredPlan {
    ...
    .map((specification, sequence) => {
      ...
      const preimageBlob =
        specification.preimage.exists && specification.preimage.type === "file"
          ...
          : null;
      if (desiredBlob !== null) {
        if (specification.desiredContent === undefined)
          throw new Error("missing desired bytes");
        desired[desiredBlob] = Buffer.from(
          specification.desiredContent,
        ).toString("base64");
        aggregateBlobBytes += Buffer.byteLength(specification.desiredContent);
      }
      if (preimageBlob !== null) {
        if (specification.preimageContent === undefined)
          throw new Error("missing preimage bytes");
        preimage[preimageBlob] = Buffer.from(
          specification.preimageContent,
        ).toString("base64");
        aggregateBlobBytes += Buffer.byteLength(specification.preimageContent);
      }
      const target = specification.desired.exists
        ? specification.desired
        : specification.preimage;
...
async function closeCommitted(
  ...
  receipt = (await controller.replace(
    receipt,
    authorityVerified,
...
describe("authored state transaction", () => {
  ...
  it("rejects wrong leases and forged, stale, or reused materialization authority", async () => {
    ...
    const staleAuthority = await controller.authorizeAuthoredState(
      intentReceipt,
      lease,
    );
    ...
    expect(() =>
      controller.assertAuthoredStateAuthority(staleAuthority, projection),
    ).toThrow(/stale or foreign/iu);

    const authority = await controller.authorizeAuthoredState(
      handedOffReceipt,
      lease,
    );
  ...
  it("removes an exact marker-owned partial preparation and rematerializes idempotently", async () => {
    ...
    await expect(
      commitAuthoredState(resumed.transaction),
  ...
  it.each([
    ...
    "recovers a pending authored preparation interrupted at %s",
    async (boundary) => {
      const root = projectRoot();
  ...
  it.each([
    ...
    async ({ linked, recovery }) => {
      ...
      const bytes = linked
        ...
        : plan.replayManifestBytes.slice(
            0,
            Math.floor(plan.replayManifestBytes.length / 2),
          );
  ...
  it("resumes an abort after owned cleanup and aborted-postcondition crash windows", async () => {
    for (const abortBoundary of [
      ...
      let claimed = (await controller.claim(
        OPERATION_ID,
      )) as DurableOpenPromotionJournal;
      await expect(
        abortPendingAuthoredPreparation({
      ...
      await expect(
        controller.verifyOpen(resumed.receipt),
      ).resolves.toMatchObject({
  ...
  it("advances zero-mutation commit and rollback phases explicitly", async () => {
    ...
    const committed = await commitAuthoredState(harness.prepared.transaction);
    await expect(
      harness.controller.verifyOpen(committed.receipt),
    ).resolves.toMatchObject({
      progress: { phase: "authored-committed", authoredCursor: 0 },
    });
    const rolledBack = await rollbackAuthoredState(
      harness.prepared.transaction,
    );
    await expect(
      harness.controller.verifyOpen(rolledBack.receipt),
    ).resolves.toMatchObject({
      progress: {
        direction: "rollback",
        phase: "authored-rolled-back",
  ...
  it("fails before a target intent when its recorded preimage changed", async () => {
    ...
    await expect(
      commitAuthoredState(harness.prepared.transaction),
    ).rejects.toThrow(/changed before its write-ahead intent/iu);
    const journal = await harness.controller.verifyOpen(
      harness.prepared.receipt,
    );
  ...
  it("fails closed on a tampered blob or replay manifest", async () => {
    ...
    await expect(
      commitAuthoredState(harness.prepared.transaction),
    ).rejects.toThrow(/blob was changed/iu);
  ...
  it("cleans only exact journal-owned artifacts and preserves a changed owner marker", async () => {
    ...
    await expect(
      cleanupAuthoredState(harness.prepared.transaction, closed),
    ).rejects.toThrow(/owner marker/iu);
    expect(existsSync(join(root, owned.authoredStage.basename))).toBe(true);
  });
  ...
  it("preserves a post-close blob whose digest or mode no longer matches", async () => {
    ...
    await expect(
      cleanupAuthoredState(harness.prepared.transaction, closed),
    ).rejects.toThrow(/incomplete authored blob is changed/iu);
  ...
  it("removes exact closed leftovers and supports receipt rebinding", async () => {
    ...
    const harness = await prepareHarness(plan, root);
    await bindAuthoredStateReceipt(
      harness.prepared.transaction,
      harness.prepared.receipt,
    );
    const committed = await commitAuthoredState(harness.prepared.transaction);
    const closed = await closeCommitted(harness.controller, committed.receipt);
    const result = await cleanupAuthoredState(
      harness.prepared.transaction,
      closed,
    );
    const owned = createRuntimePromotionOwnedSlots(OPERATION_ID);
  ...
  it("resumes cleanup after an authored stage was removed before its postcondition", async () => {
    ...
    await expect(
      cleanupAuthoredState(harness.prepared.transaction, closed),
    ).rejects.toThrow(/cleanup crash/iu);
  ...
  it("resumes cleanup after replay unlink without needing current renderers or bytes", async () => {
    ...
    await expect(
      cleanupAuthoredState(harness.prepared.transaction, closed),
    ).rejects.toThrow(/replay cleanup crash/iu);
