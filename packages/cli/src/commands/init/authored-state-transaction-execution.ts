...
function blobPath(
  ...
  const name =
    direction === "commit" ? mutation.desiredBlob : mutation.preimageBlob;
  if (name === null)
    authoredTransactionFailure("a file target has no durable blob");
...
function assertReconcilableCurrent(
  ...
  if (
    ...
    authoredTransactionFailure(
      "a target no longer matches its recorded preimage",
    );
  }
}
...
export function reconcileAuthoredMutation(
  ...
  if (!sameAuthoredPathState(verified, target)) {
    authoredTransactionFailure(
      "an authored target did not reach its exact postcondition",
    );
  }
  return "applied";
}
...
export function verifyEveryAuthoredTarget(
  ...
  for (const mutation of manifest.mutations) {
    ...
    if (
      !sameAuthoredPathState(observeAuthoredPath(root, mutation.path), expected)
    ) {
