...
const BRIDGE_MISSING = hostErrorCatalog.require('CLI.CAPABILITY_WORKER.NO_ISOLATION_BRIDGE');
    ...
    throw new PluginIncompatibleError(
      `capability domain '${args.domainId}' does not support isolated external packages`,
      {
        ...
        metadata: {
          ...
          ownerToolId: args.owningTool.metadata.name ?? args.owningTool.metadata.id,
        },
