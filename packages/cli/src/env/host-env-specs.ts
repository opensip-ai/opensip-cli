...
export const CLI_INFRA_ENV_SPECS: readonly EnvVarSpec<unknown>[] = [
  ...
  },
  {
    ...
    docs:
      ...
      'The default allow-list also forwards the CLI tool admission controls ' +
      '(OPENSIP_CLI_ALLOW_*_TOOLS and OPENSIP_CLI_SKIP_*), so a worker sees the same ' +
      'explicit trust decisions as the supervising process. ' +
...
export const BUNDLED_TOOL_ENV_SPECS: readonly EnvVarSpec<unknown>[] = [
  ...
  },
  {
    ...
    docs:
      ...
      "child so the child's plain --json path skips inline signal delivery (no per-unit cloud " +
      'egress or verdict exit — the parent owns the aggregate, audit P1-2). Never set by hand.',
  },
];
...
export const PRE_SCOPE_ENV_SPECS: readonly EnvVarSpec<unknown>[] = [
  ...
  {
    canonical: 'FORCE_COLOR',
    docs: 'Force ANSI colours even when the stream is not a TTY.',
  },
  {
    canonical: 'COLORTERM',
    docs: 'Terminal colour capability hint (e.g. truecolor).',
  },
