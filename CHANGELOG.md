...
| Init recovery finds state that disagrees with its durable evidence                  |      1 |     2 | Now `CLI.INIT.RECOVERY_EVIDENCE_MISMATCH` (`integrity`, `configuration`). ~26 recovery refusals whose specific messages were replaced by "The operation failed." after an interrupted `init`.                                       |
| Runtime-promotion source fails authority verification                               |      1 |     2 | Now `CLI.INIT.PROMOTION_SOURCE_UNVERIFIED` (`integrity`, `configuration`). The raising class extended bare `Error`, so a failure that decides whether the runtime lease may be released reported as an unknown internal fault.      |
| Init authored-state transaction refuses to proceed                                  |      1 |     2 | Now `CLI.INIT.AUTHORED_TRANSACTION_UNSAFE` (`integrity`, `configuration`). ~199 refusals across the authored-state plane whose specific messages were replaced by "The operation failed."; each now names the invariant that broke. |
| Uninstall refuses a symlinked or non-directory project root                         |      1 |     2 | Now `CLI.UNINSTALL.REFUSED_PROJECT_ROOT` (`security`, `configuration`). The refusal message names the fix ("Pass a real directory"); it previously reached the user as "The operation failed."                                      |
| Uninstall refuses a symlinked managed directory                                     |      1 |     2 | Now `CLI.UNINSTALL.REFUSED_SYMLINK_LEAF`. Same reasoning: the operator replaces the link and re-runs.                                                                                                                               |
| Uninstall refuses a symlinked or non-directory user root                            |      1 |     2 | Now `CLI.UNINSTALL.REFUSED_SYMLINK_USER_ROOT`.                                                                                                                                                                                      |
| Uninstall refuses a removal target outside the allowed roots                        |      1 |     2 | Now `CLI.UNINSTALL.TARGET_ESCAPES_ROOTS` (`integrity`): the containment invariant guarding the recursive delete. Nothing is removed.                                                                                                |
| User-level uninstall stops in a state it will not modify further                    |      1 |     2 | Now `CLI.UNINSTALL.USER_RECOVERY_UNSAFE`: ~22 fail-closed conditions funnel through one refusal that previously carried no code at all.                                                                                             |
...
| Graph project root cannot be inspected due to filesystem access or resource failure |      2 |     1 | The old blanket `ConfigurationError` mislabeled every `stat` failure as a missing directory; native permission/resource and unclassified I/O faults now retain an environmental runtime classification.                             |
| Fitness recipe execution cannot find a requested recipe                             |      1 |     3 | The outer fitness failure funnel now uses the canonical mapper, so the service's registered `FIT.NOT_FOUND.RECIPE` definition retains its not-found exit instead of becoming a generic runtime failure.                             |
| Fitness recipe execution is cooperatively cancelled                                 |      1 |   130 | The fitness failure funnel now preserves `CORE.SYSTEM.CANCELLED` instead of flattening every non-configuration failure to runtime exit 1.                                                                                           |
| Simulation recipe execution is cooperatively cancelled                              |      1 |   130 | Cancellation now escapes the per-scenario result channel and reaches the host as `CORE.SYSTEM.CANCELLED`; it is no longer reported as a faulted scenario.                                                                           |
| Simulation assertion names an unknown runtime metric                                |      0 |     1 | Runtime-authored scenario metric typos now fail closed as `SIMULATION.SCENARIO.INVALID` instead of resolving to zero and allowing low-error assertions to false-green.                                                              |
| YAGNI command cannot find an explicitly selected detector                           |      2 |     3 | The selector now raises registered `YAGNI.DETECTOR.NOT_FOUND`, distinguishing a missing detector from malformed configuration.                                                                                                      |
| YAGNI detector execution is cooperatively cancelled                                 |      0 |   130 | YAGNI now runs detectors on the shared cancellation substrate, and its advisory exit hook no longer overwrites the host interrupt coordinator's cancelled exit with success.                                                        |
| ----------------------------------------------------------------------------------- | -----: | ----: | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Host refuses to edit `tools.trusted` (malformed/oversized/non-map config)          |      1 |     2 | Now `CLI.TOOLS.EDIT_REFUSED` (`integrity`, `configuration`): the operator fixes the config file, so this is a configuration failure rather than an unknown internal failure.                           |
| `tools install` cannot resolve a safe npm-pack tarball name                        |      1 |     2 | Now `CLI.TOOLS.PACK_INVALID` (`configuration`).                                                                                                                                                       |
| `tools install` detects a packed tarball outside the staged package directory      |      1 |     2 | Now `CLI.TOOLS.PACK_CONTAINMENT` (`security`, `configuration`): containment refusal is operator-actionable.                                                                                           |
| CPU-profile artifact path escapes containment or is not a real directory           |      1 |     2 | Now `CLI.PROFILE.CONTAINMENT_REFUSED` (`security`, `configuration`).                                                                                                                                  |
| Runtime command inventory rejects a tool/host commandSpecs shape                   |      1 |     5 | Now `CLI.RUNTIME_INVENTORY.INVALID` (`plugin-incompatible`): plugin authors fix the declared command surface.                                                                                         |
| Capability worker denies an undeclared resource                                    |      1 |     5 | Now `CLI.CAPABILITY_WORKER.RESOURCE_DENIED` (`security`, `plugin-incompatible`).                                                                                                                             |
| Capability isolation bridge is missing for a domain that requires a worker         |      1 |     5 | Now `CLI.CAPABILITY_WORKER.NO_ISOLATION_BRIDGE` (`plugin-incompatible`).                                                                                                                                          |
| Host RPC requests a host plane this host does not provide                          |      1 |     2 | Now `CLI.HOST.PLANE_UNAVAILABLE` (`configuration`): OSS hosts omit planes deliberately; the caller must stop using them or use a host that implements them.                                           |
| Data-gathering hook worker calls a denied host output/render seam                  |      1 |     5 | Now `CLI.HOST.SEAM_DENIED` (`security`, `plugin-incompatible`).                                                                                                                                       |
| Duplicate session-replay contribution for one tool short id                        |      1 |     5 | Now `CLI.SESSION_REPLAY.DUPLICATE` (`plugin-incompatible`).                                                                                                                                           |
| External session replay cannot be isolated and is refused in-process               |      1 |     5 | Now `CLI.SESSION_REPLAY.ISOLATION_REFUSED` (`security`, `plugin-incompatible`).                                                                                                                       |
